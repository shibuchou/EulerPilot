...
`security_policy_demo.bpf.c` 是当前 SecurityPolicySkill 的最小 BPF 演示程序，包含 `lsm/file_open` 文件访问强制控制、`lsm/bprm_check_security` 程序执行强制控制、`lsm/socket_connect` IPv4 endpoint 强制控制、`lsm/ptrace_traceme` scoped ptrace 强制控制、`lsm/capable` scoped capability 强制控制、`lsm/task_fix_setuid` scoped setuid credential 强制控制、`lsm/task_fix_setgid` scoped setgid credential 强制控制、`lsm/task_fix_setgroups` scoped supplementary groups 强制控制和 `sys_enter_execve/sys_enter_openat/sys_enter_connect/sys_enter_ptrace` tracepoint 观测。它已经提供 `policy_map.enforce/target_count`、最多 8 项 `BPF_ARRAY target_map` 和 ringbuf 命中事件：audit 模式允许访问并记录 `observed`，同时输出 `lsm_file_open`、`lsm_bprm_check_security` 与四类 syscall 事件；enforce 模式在用户态写入的 path/path_prefix/file_access/exec/exec_prefix/socket/ptrace/capability/setuid/setgid/setgroups 目标上拒绝访问并记录 `blocked`。`target_map` 每项可选 `cgroup_id`，未配置时按路径、路径前缀或 endpoint 匹配，配置后只对当前进程 cgroup id 命中的目标生效；ptrace、capable、task_fix_setuid、task_fix_setgid 与 task_fix_setgroups 规则只接受 scoped cgroup target，不允许全局阻断。`file_access` 支持 `any/read/write`，默认 `any` 兼容原路径阻断；`write` 模式只匹配 `O_WRONLY/O_RDWR`，可与精确 `path` 或目录 `path_prefix` 组合，用于验证目标 cgroup 内“可读但不可写”的文件或只读目录策略。LSM file/bprm/socket/ptrace/capable/credential 事件会携带 `target_index`，用户态据此还原单条 YAML `rule_id/target_ref`；file_open 事件额外携带 `path_prefix/file_access/file_flags`，socket 事件额外携带 `dst_ip/dst_port/protocol`，exec_prefix 事件额外携带用户态回填的 `exec_prefix`，ptrace 事件携带 `path=ptrace_traceme`，capable 事件携带 capability 编号并由用户态输出为 `CAP_SYS_ADMIN` 等名称，task_fix_setuid 事件携带 `uid/euid/suid/setuid_flags`，task_fix_setgid 事件携带 `gid/egid/sgid/setgid_flags`，task_fix_setgroups 事件携带 `group_count/old_group_count`；tracepoint 观测事件统一写 unknown target，并可被用户态 `anomaly_rules` 复用；当前 `burst_execve` 规则只在用户态做窗口聚合，BPF 热路径不承担复杂异常判断。`bprm_check_security` 优先从 trusted `bprm->file` 解析执行文件路径，若未命中再回退到 `bprm->filename`，以兼容脚本解释器路径和用户态传入路径差异；匹配逻辑同时支持精确 `exec_path` 与字面 `exec_prefix`。当前 `target_map` 已支持多 path/path_prefix/file_access/exec/exec_prefix/socket/ptrace/capability/setuid/setgid/setgroups demo 目标和显式 cgroup scope；正式说明和最小测试入口见 `docs/security_policy_skill.md` 与 `tests/integration/test_security_policy.sh`。
