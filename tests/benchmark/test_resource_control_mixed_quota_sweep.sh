...
RPS_RETENTION_MIN="${RPS_RETENTION_MIN:-0.70}"
RESULT_DIR="${RESULT_DIR:-results/resource_control/mixed-quota-sweep-$(date +%Y%m%d-%H%M%S)}"
...
http {
    ...
    server {
        listen $NGINX_PORT;
        location / {
            return 200 'EulerPilot mixed resource-control quota sweep\n';
        }
    }
}
EOF_NGINX
...
rows = []
no_quota_get = redis_rps("no_quota", "GET")
no_quota_set = redis_rps("no_quota", "SET")
no_quota_nginx = nginx_rps("no_quota")
...
for label in labels:
    ...
    nginx_ratio = nginx / no_quota_nginx if no_quota_nginx > 0 else 0.0
    ...
    rows.append({
        ...
        "redis_get_ratio_vs_no_quota": f"{get_ratio:.4f}",
        "redis_set_ratio_vs_no_quota": f"{set_ratio:.4f}",
        "nginx_rps_ratio_vs_no_quota": f"{nginx_ratio:.4f}",
        "business_min_ratio_vs_no_quota": f"{business_min_ratio:.4f}",
...
eligible = [
    ...
    and float(row["business_min_ratio_vs_no_quota"]) >= rps_retention_min
]
if eligible:
    recommended = min(eligible, key=lambda row: float(row["background_ratio_vs_no_quota"]))
    recommendation_reason = f"all_business_retention_ge_{rps_retention_min:.2f}_and_min_background_ratio"
else:
    quota_rows = [row for row in rows if row["label"] != "no_quota"]
    recommended = max(
        quota_rows,
        key=lambda row: (
            float(row["business_min_ratio_vs_no_quota"]),
            -float(row["background_ratio_vs_no_quota"]),
        ),
    )
    recommendation_reason = "no_profile_met_all_business_retention_threshold"

quota_10 = next(row for row in rows if row["label"] == "quota_10")
if no_quota_get <= 0 or no_quota_set <= 0 or no_quota_nginx <= 0:
    raise SystemExit("no_quota Redis/Nginx throughput is not positive")
...
with summary.open("w") as f:
    f.write("result=pass\n")
    f.write("benchmark=mixed_redis_nginx_background_cpu_quota_sweep\n")
    f.write(f"rps_retention_min={rps_retention_min:.2f}\n")
    f.write(f"no_quota_redis_get_rps={no_quota_get:.2f}\n")
    f.write(f"no_quota_redis_set_rps={no_quota_set:.2f}\n")
    f.write(f"no_quota_nginx_requests_per_sec={no_quota_nginx:.2f}\n")
    ...
    f.write(f"quota_10_business_min_ratio_vs_no_quota={quota_10['business_min_ratio_vs_no_quota']}\n")
    ...
    f.write(f"recommended_business_min_ratio_vs_no_quota={recommended['business_min_ratio_vs_no_quota']}\n")
    f.write(f"recommended_redis_get_ratio_vs_no_quota={recommended['redis_get_ratio_vs_no_quota']}\n")
    f.write(f"recommended_redis_set_ratio_vs_no_quota={recommended['redis_set_ratio_vs_no_quota']}\n")
    f.write(f"recommended_nginx_rps_ratio_vs_no_quota={recommended['nginx_rps_ratio_vs_no_quota']}\n")
...
with report.open("w") as f:
    f.write("# Mixed Redis + Nginx Background CPU Quota Sweep Benchmark\n\n")
    f.write("- result: `pass`\n")
    f.write(f"- all-business retention threshold: `{rps_retention_min:.2f}`\n")
    ...
    f.write("| Profile | cpu.max | Redis GET Ratio | Redis SET Ratio | Nginx RPS Ratio | Business Min Ratio | Nginx p99 | Background Ratio | nr_throttled | throttled_usec |\n")
    f.write("|---------|---------|-----------------|-----------------|-----------------|--------------------|-----------|------------------|--------------|----------------|\n")
    for row in rows:
        f.write(
            f"| {row['label']} | `{row['cpu_max']}` | {row['redis_get_ratio_vs_no_quota']} | "
            f"{row['redis_set_ratio_vs_no_quota']} | {row['nginx_rps_ratio_vs_no_quota']} | "
            f"{row['business_min_ratio_vs_no_quota']} | {row['nginx_p99_latency']} | "
            f"{row['background_ratio_vs_no_quota']} | {row['nr_throttled_delta']} | "
            f"{row['throttled_usec_delta']} |\n"
        )
    f.write("\n## Interpretation\n\n")
    f.write("This benchmark runs Redis and Nginx pressure at the same time while keeping the Agent placement path consistent. It sweeps only the background `cpu.max` pressure profile and uses Redis GET/SET plus Nginx RPS as foreground boundary signals. The pass condition focuses on proving background CPU suppression and throttling, while the recommendation records which profile keeps all foreground ratios above the configured retention threshold when possible.\n\n")
    f.write("## Artifacts\n\n")
...
[ "$(id -u)" -eq 0 ] || fail 'resource_control mixed quota sweep benchmark must run as root'
mkdir -p "$RESULT_DIR"
...
info "resource_control mixed quota sweep benchmark result saved to $RESULT_DIR"
