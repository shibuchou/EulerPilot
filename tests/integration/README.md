...
- `test_network_xdp.sh`：验证 `network_xdp` 子能力和 schema v2 `netdev target_ref`，创建专用 netns/veth，确认 audit 不挂 XDP，enforce 以 generic XDP drop ICMP、TCP:19092、UDP:19093，并在 rollback 事件中校验 per-rule drop 统计，最后确认 XDP detached 且连通性恢复。脚本只使用 Python 标准库，不依赖 PyYAML。121 当前结果：`results/network_policy/xdp-20260703-121-udp-v2`；122 当前结果：`results/network_policy/xdp-20260703-122-udp-v2`。
