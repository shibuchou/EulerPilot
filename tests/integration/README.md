...
- `test_target_resolver.sh`：验证 `TargetResolver` 的 netdev 解析、`k8s_pod` 错误路径，以及 root 环境下基于临时 netns/veth + fake `kubectl/crictl` 的 Pod host veth 解析路径；不依赖真实 Kubernetes。
