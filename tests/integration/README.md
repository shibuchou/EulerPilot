...
- `test_security_policy.sh`：验证正式 `security_policy` 注册名、schema v2 path target、audit 模式不阻断并写 AuditBus、enforce 模式 BPF LSM attach 后目标文件拒绝、Agent 退出恢复和 cleanup 无残留；`security_policy_demo` 作为兼容名保留。当前 BPF demo 硬编码 `/root/EulerPilot`，在其他路径会安全跳过。
