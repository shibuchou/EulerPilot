...
- `test_security_policy.sh`：验证正式 `security_policy` 注册名、schema v2 path/path_prefix/file_access/exec/exec_prefix/socket/ptrace/capability/setuid/setgid/setgroups target、用户态从 YAML target path、path_prefix、file_access、exec path、exec_prefix、dst_ip、dst_port、capability、可选 cgroup_path、`type: pid`、`type: container_id`、`type: container` 和 `type: k8s_pod` 解析结果填充最多 8 项 BPF `target_map`、audit 模式 attach BPF 但不阻断，并写入 `lsm_file_open`、`lsm_bprm_check_security`、`sys_enter_execve`、`sys_enter_openat`、`sys_enter_connect`、`sys_enter_ptrace` 等 ringbuf hit 事件；随后用 `anomaly_rules: burst_execve` 验证 `sys_enter_execve` 用户态速率聚合，输出 `security_policy_events.anomaly-execve.jsonl`；enforce 模式 BPF LSM attach 后目标文件、demo 可执行文件、目标 cgroup 内 exec_prefix 匹配的可写目录脚本、目标 cgroup 内 `file_access=write` 精确路径写打开、目标 cgroup 内 `path_prefix + file_access=write` 只读目录写打开、scoped IPv4 socket connect、scoped `PTRACE_TRACEME`、scoped `CAP_SYS_ADMIN`、scoped `setuid(65534)`、scoped `setgid(65534)` 和 scoped `setgroups([65534])` 拒绝并写 blocked hit 事件；随后创建 `/tmp/eulerpilot-security-policy.*` 下两组动态目标，验证 YAML `path/exec_path` 真实驱动多目标 `target_map`，且 LSM blocked 事件分别带上对应的 `rule_id/target_ref`，原 demo 目标不会被误阻断；再创建临时 cgroup 验证显式 cgroup、PID、container_id、runtime container name 和 k8s pod name 都能解析到 cgroup scope，且 scope 外允许、scope 内阻断、事件带 `cgroup_id/cgroup_path`；socket 用例启动本地 TCP server，验证 `lsm_socket_connect` blocked 事件带 `dst_ip/dst_port/protocol`；exec_prefix 用例验证 blocked 事件带 `exec_prefix`；file_access 用例验证读打开成功、写打开失败且事件带 `file_access/file_flags`；path_prefix 用例验证目录前缀下读打开成功、写打开失败且事件带 `path_prefix/file_access/file_flags`；ptrace 用例验证 `lsm_ptrace_traceme` blocked 事件带 `path=ptrace_traceme` 和 cgroup 证据；capable 用例验证 `lsm_capable` blocked 事件带 `capability=CAP_SYS_ADMIN` 和 cgroup 证据；setuid 用例验证 `lsm_task_fix_setuid` blocked 事件带 `uid/euid/suid/setuid_flags` 和 cgroup 证据；setgid 用例验证 `lsm_task_fix_setgid` blocked 事件带 `gid/egid/sgid/setgid_flags` 和 cgroup 证据；setgroups 用例验证 `lsm_task_fix_setgroups` blocked 事件带 `group_count/old_group_count` 和 cgroup 证据；Agent 退出恢复和 cleanup 无残留；`security_policy_demo` 作为兼容名保留。当前脚本以 `/root/EulerPilot` 作为 Agent BPF object 与 demo 结果目录基准，在其他路径会安全跳过。121 最新通过结果：`results/security_policy/integration-20260623-214457`；122 最新通过结果：`results/security_policy/integration-20260623-214850`。
