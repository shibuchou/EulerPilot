...
- `test_network_xdp.sh`：验证 `network_xdp` 子能力和 schema v2 `netdev target_ref`，创建专用 netns/veth，确认 audit 不挂 XDP，enforce 以 generic XDP drop ICMP、TCP:19092、UDP:19093 和 UDP tuple `10.89.0.2:39094 -> 10.89.0.1:19094`，并在 rollback 事件中校验 per-rule `protocol/src_ip/dst_ip/src_port/dst_port/drop_count/byte_count` 统计，最后确认 XDP detached 且连通性恢复。脚本只使用 Python 标准库，不依赖 PyYAML。121 当前结果：`results/network_policy/xdp-20260703-121-fields-v1`；122 当前结果：`results/network_policy/xdp-20260703-122-fields-v1`。
- `test_network_xdp_real_pod_veth.sh`：真实 Kubernetes lab Pod host veth XDP 演示入口。`kubectl`、CRI 和 `eulerpilot-lab` demo Pod 可用时，通过 `type: k8s_pod + namespace + pod_name` 解析 Pod host veth，在该 veth 上挂 generic XDP，使用宿主机 `nsenter` 进入 Pod netns 发送 Pod-to-bridge ICMP、TCP:19092、UDP:19093 和 UDP tuple `pod_ip:39094 -> bridge_ip:19094` 流量，验证四条规则 per-rule `XDP_DROP` 命中、字段证据和 rollback detach。121 当前结果：`results/network_policy/real-pod-veth-xdp-20260703-k3s-121-tuple-v1`；122 当前结果：`results/network_policy/real-pod-veth-xdp-20260703-k3s-122-tuple-v1`。
...
- `test_security_policy_anomaly_rules.sh`：验证 v3.1 服务联动 anomaly 规则，启动正式 `security_policy` audit 模式后触发本地 `burst_connect`、`burst_openat_sensitive` 和 scoped `capability_abuse`，结果只保存 anomaly JSONL 摘要，避免把无关系统路径写入提交证据。121 最新通过结果：`results/security_policy/anomaly-rules-20260703-121-v4`；122 最新通过结果：`results/security_policy/anomaly-rules-20260703-122-v2`。
- `test_security_policy_credential_anomaly.sh`：验证 `credential_churn` credential 生命周期 anomaly 规则，启动正式 `security_policy` audit 模式后在 scoped cgroup 内重复触发 `setuid(65534)`，确认 `lsm_cred_prepare/lsm_task_fix_setuid` 事件聚合为 anomaly，且 evidence 携带 `credential_stage`、`uid` 或 `cred_gfp` 等生命周期字段。当前脚本以 `/root/EulerPilot` 为基准，在其他路径会安全跳过。121 最新通过结果：`results/security_policy/credential-anomaly-20260703-121-v3`；122 最新通过结果：`results/security_policy/credential-anomaly-20260703-122-v3`。
