...
- `test_network_xdp_real_pod_veth.sh`：真实 Kubernetes lab Pod host veth XDP 演示入口。`kubectl`、CRI 和 `eulerpilot-lab` demo Pod 可用时，通过 `type: k8s_pod + namespace + pod_name` 解析 Pod host veth，在该 veth 上挂 generic XDP，使用宿主机 `nsenter` 进入 Pod netns 发送 Pod-to-bridge ICMP、TCP:19092、UDP:19093 流量，验证三条规则 per-rule `XDP_DROP` 命中和 rollback detach。121 当前结果：`results/network_policy/real-pod-veth-xdp-20260703-k3s-121-udp-v4`；122 当前结果：`results/network_policy/real-pod-veth-xdp-20260703-k3s-122-udp-v4`。
