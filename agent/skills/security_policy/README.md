...
- 更多 LSM hook 和更复杂异常策略组合；`cred_alloc_blank/cred_transfer` 已完成 scoped attach 评估，但普通用户态 `setuid/fork/unshare` workload 未稳定触发 runtime hit，因此报告只写“已支持/已评估/已记录 hit 计数”，不写成稳定业务命中。
