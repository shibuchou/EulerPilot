...
- BPF 程序：`bpf/security_policy.bpf.c`
...
`anomaly_rules` 支持在用户态聚合阶段增加进程过滤，不新增 BPF hook：

- `comm` / `process_comm`：精确匹配 BPF ringbuf 事件中的 `comm`。
- `comm_prefix` / `process_comm_prefix`：前缀匹配，适合 `python`、`redis`、`nginx` 等版本号或包装器可能变化的进程。

双机结果 `anomaly-process-filter-20260703-121-v1` 和 `anomaly-process-filter-20260703-122-v1` 验证：`comm_prefix=python` 的 `/etc` openat burst 规则输出 anomaly，`comm=nohit-proc` 的负向规则不输出 anomaly。

## v3.2 combo scope filter

`anomaly_rules` 还支持 `target_ref` / `scope_target_ref`，用于把异常聚合限定在已解析的 target scope 内。配置该字段后，只有 BPF 事件能通过 `target_index` 回映射到指定 `target_ref` 时才进入窗口计数。

双机结果 `anomaly-combo-scope-20260703-121-v1` 和 `anomaly-combo-scope-20260703-122-v1` 验证：scope 外 Python `/etc` open burst 不触发；进入目标 cgroup 后，同一条规则同时匹配 `syscall=openat`、`path_prefix=/etc`、`comm_prefix=python` 和 `target_ref=scoped_python_etc` 才输出 anomaly，并携带 `target_ref_filter`、`target_ref` 与 `cgroup_id` 证据。
