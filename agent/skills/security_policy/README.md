...
- 用户态：`SecurityPolicyDemoSkill` 同时服务 `security_policy` 和 `security_policy_demo`；正式名读取 YAML v2 `targets + rules + target_ref`，并从最多 8 条规则引用的 `targets.<target_ref>.path`、`exec_path` 和可选 `cgroup_path` 填充 BPF `target_map`；LSM file/bprm blocked 事件会根据 BPF `target_index` 映射回单条 YAML `rule_id/target_ref`，并在 cgroup scoped target 命中时输出 `cgroup_id/cgroup_path`；legacy 兼容入口仍支持 `target_path` / `target_exec_path`
...
- enforce：通过 libbpf 打开 `/root/EulerPilot/build/security_policy_demo.bpf.o` 并 attach LSM + tracepoint；当前 `lsm/file_open` 和 `lsm/bprm_check_security` 会对 `target_map` 中最多 8 组 path/exec/cgroup scope 目标返回拒绝；未配置 `cgroup_path` 时兼容原来的路径匹配，配置后只对目标 cgroup 生效
- 回滚：Agent stop/rollback 销毁所有 BPF link 和 object；正常路径不 pin link

当前尚未完成：

- Pod/container ID 到 cgroup 的自动解析和进程过滤
