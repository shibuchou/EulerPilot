...
v3.2 对 credential 生命周期先补可稳定验收的用户态 anomaly 规则：
...
- 安全边界：不新增全局 credential enforce，不启用 `cred_transfer/cred_alloc_blank` 等更热的 hook；这些 hook 仍作为后续增强项，必须继续复用 explicit cgroup/Pod/container scope。
