...
OUTROOT="$ROOT/results/reports/trigger-sweep-incr-$(date +%Y%m%d-%H%M%S)"
RUNS="${RUNS:-2}"
...
PSI_THRESHOLDS="${PSI_THRESHOLDS:-0.05}"
WAIT_THRESHOLDS_NS="${WAIT_THRESHOLDS_NS:-500000 1000000 1500000}"
BACKGROUND_RUNTIME_THRESHOLDS_NS="${BACKGROUND_RUNTIME_THRESHOLDS_NS:-2500000 3000000 3500000}"
...
for psi in $PSI_THRESHOLDS; do
    for wait_ns in $WAIT_THRESHOLDS_NS; do
        for bg_ns in $BACKGROUND_RUNTIME_THRESHOLDS_NS; do
            printf '[INFO] incr-refine cpu_psi_threshold=%s latency_wait_threshold_ns=%s background_runtime_threshold_ns=%s\n' "$psi" "$wait_ns" "$bg_ns"
...
printf '[INFO] incr-refine sweep complete: %s\n' "$OUTROOT"
