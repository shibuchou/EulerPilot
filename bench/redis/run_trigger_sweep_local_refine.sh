...
OUTROOT="$ROOT/results/reports/trigger-sweep-local-$(date +%Y%m%d-%H%M%S)"
RUNS="${RUNS:-2}"
...
PSI_THRESHOLDS="${PSI_THRESHOLDS:-0.03 0.05 0.07}"
WAIT_THRESHOLDS_NS="${WAIT_THRESHOLDS_NS:-1000000 2000000 3000000}"
BACKGROUND_RUNTIME_THRESHOLDS_NS="${BACKGROUND_RUNTIME_THRESHOLDS_NS:-3000000 4000000 5000000}"
...
for psi in $PSI_THRESHOLDS; do
    for wait_ns in $WAIT_THRESHOLDS_NS; do
        for bg_ns in $BACKGROUND_RUNTIME_THRESHOLDS_NS; do
            printf '[INFO] refine cpu_psi_threshold=%s latency_wait_threshold_ns=%s background_runtime_threshold_ns=%s\n' "$psi" "$wait_ns" "$bg_ns"
...
printf '[INFO] local trigger refine complete: %s\n' "$OUTROOT"
