...
int main(int argc, char **argv)
{
	...
	skel = usage_bpf__open_and_load(); //加载和验证BPF应用程序
	if (!skel) {
		fprintf(stderr, "Failed to open BPF skeleton\n");
		return 1;
	}
	/* Attach tracepoint handler */
	err = usage_bpf__attach(skel); //参数代表了已加载和初始化的BPF程序对象
	if (err) { // 操作失败
	...
	pb = perf_buffer__new(bpf_map__fd(skel->maps.perf), 8 /* 32KB per CPU */, handle_event,
			      NULL, NULL, NULL);
...
cleanup:
	usage_bpf__destroy(skel); //销毁之前创建的BPF程序对象 (skel), 确保释放相关资源
