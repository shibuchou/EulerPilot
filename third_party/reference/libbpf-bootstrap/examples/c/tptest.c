...
void handle_event(void *ctx, int cpu, void *data, unsigned int data_sz)
{
    ...
    printf("[Runtime:%d]\n",md->cpu);
    printf(" pid=%d  runtime=%lu [ns] vruntime=%lu [ns]  \n",  md->pid,
                md->runtime, md->vruntime);	
   
}

int main(int argc, char **argv) {
    struct tptest_bpf *skel;
    int err ;
    bump_memlock_rlimit();
    ...
    skel = tptest_bpf__open();
    if (!skel) {
		fprintf(stderr, "Failed to open and verify BPF skeleton\n");
		goto cleanup;
	}

    err = tptest_bpf__load(skel);
	if (err) {
		fprintf(stderr, "Failed to load BPF skeleton\n");
		goto cleanup;
	}

    err = tptest_bpf__attach(skel);
	if (err) {
