...
void handle_event(void *ctx, int cpu, void *data, unsigned int data_sz)
{
    ...
    printf( "   %2d     %-5d    %s        %ld         %ld        %ld          %ld        %ld\n",md->cpu, md->pid, str,md->total_vm,md->data,md->text,md->shared,md->resident); 
      
}

int main(int argc, char **argv) {
    struct pidmm2_bpf *skel;
    int err ;
    bump_memlock_rlimit();
    ...
    skel = pidmm2_bpf__open();
    if (!skel) {
		fprintf(stderr, "Failed to open and verify BPF skeleton\n");
		goto cleanup;
	}

    err = pidmm2_bpf__load(skel);
	if (err) {
		fprintf(stderr, "Failed to load BPF skeleton\n");
		goto cleanup;
	}

    err = pidmm2_bpf__attach(skel);
	if (err) {
