...
本目录是 SecurityPolicySkill 的最小 BPF 演示场景，不依赖 Kubernetes。当前 demo 验证三件事：Agent attach `bpf/security_policy_demo.bpf.c` 后，`lsm/file_open` 可以在 enforce 模式拒绝读取固定文件；`lsm/bprm_check_security` 可以在 enforce 模式拒绝执行固定 demo 脚本；audit 模式同时通过 ringbuf 输出 `lsm_file_open`、`lsm_bprm_check_security`、`sys_enter_execve`、`sys_enter_openat`、`sys_enter_connect`、`sys_enter_ptrace` 命中事件。
...
- 当前行为：`lsm/file_open` 命中固定 secret 文件后按 mode 决定 allow 或返回 `-EPERM`；`lsm/bprm_check_security` 命中固定 demo 执行脚本后按 mode 决定 allow 或返回 `-EPERM`；四类 syscall 只观测不阻断
- 当前模式：`audit` 通过 `policy_map.enforce=0` 允许访问并记录 observed hit；`enforce` 通过 `policy_map.enforce=1` 阻断固定 secret 文件和固定 demo 执行脚本并记录 blocked hit
...
这是正式 `security_policy` 的最小兼容 demo，不是完整 SecurityPolicySkill。不能把它直接用于真实业务路径，也不能扩大成全局文件拒绝策略。下一步需要补动态 target、路径/进程/容器过滤和更完整的 ActionJournal 回滚证据。
