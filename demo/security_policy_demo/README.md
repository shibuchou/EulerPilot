...
本目录是 SecurityPolicySkill 的最小 BPF 演示场景，不依赖 Kubernetes。当前 demo 验证三件事：Agent attach `bpf/security_policy_demo.bpf.c` 后，用户态从 YAML target path 与 exec path 填充单 entry BPF `target_map`，`lsm/file_open` 可以在 enforce 模式拒绝读取目标文件；`lsm/bprm_check_security` 可以在 enforce 模式拒绝执行 demo 脚本；audit 模式同时通过 ringbuf 输出 `lsm_file_open`、`lsm_bprm_check_security`、`sys_enter_execve`、`sys_enter_openat`、`sys_enter_connect`、`sys_enter_ptrace` 命中事件。
...
- 当前行为：`lsm/file_open` 命中 `target_map` 中的 demo secret 文件后按 mode 决定 allow 或返回 `-EPERM`；`lsm/bprm_check_security` 命中 `target_map` 中的 demo 执行脚本后按 mode 决定 allow 或返回 `-EPERM`；四类 syscall 只观测不阻断
...
- 当前输出：写入 `reports/events/security_policy.jsonl`，测试会归档为 `security_policy_events.audit.jsonl`、`security_policy_events.enforce.jsonl` 和 `security_policy_events.dynamic.jsonl`
...
这是正式 `security_policy` 的最小兼容 demo，不是完整 SecurityPolicySkill。当前 `target_map` 已能由 YAML 填充单组文件/执行路径，并在集成测试中验证 `/tmp/eulerpilot-security-policy.*` 动态目标；但它仍不是生产级多目标、路径/进程/容器过滤矩阵，也不能扩大成全局文件拒绝策略。下一步需要补多目标规则 map、容器绑定和更完整的 ActionJournal 回滚证据。121 最新通过结果为 `results/security_policy/integration-20260621-161943`，122 最新通过结果为 `results/security_policy/integration-20260621-162111`。
