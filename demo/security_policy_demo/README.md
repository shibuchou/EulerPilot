...
本目录是 SecurityPolicySkill 的最小 BPF 演示场景，不依赖 Kubernetes。当前 demo 验证两件事：Agent attach `bpf/security_policy_demo.bpf.c` 后，`lsm/file_open` 可以在 enforce 模式拒绝读取固定文件；audit 模式同时通过 ringbuf 输出 `lsm_file_open`、`sys_enter_execve`、`sys_enter_openat`、`sys_enter_connect`、`sys_enter_ptrace` 命中事件。
...
- BPF hook：`lsm/file_open`、`tracepoint/syscalls/sys_enter_execve`、`tracepoint/syscalls/sys_enter_openat`、`tracepoint/syscalls/sys_enter_connect`、`tracepoint/syscalls/sys_enter_ptrace`
- 当前行为：`lsm/file_open` 命中固定路径后按 mode 决定 allow 或返回 `-EPERM`；四类 syscall 只观测不阻断
...
- audit 模式下，读取该文件仍成功，事件文件包含 `event_hook=lsm_file_open/sys_enter_execve/sys_enter_openat/sys_enter_connect/sys_enter_ptrace` 和 `result=observed`。
...
这是正式 `security_policy` 的最小兼容 demo，不是完整 SecurityPolicySkill。不能把它直接用于真实业务路径，也不能扩大成全局文件拒绝策略。下一步需要补动态 target、路径/进程/容器过滤、`bprm_check_security` enforce 和更完整的 ActionJournal 回滚证据。
