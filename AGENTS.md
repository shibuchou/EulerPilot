...
- 当前 Resource Control 已进入 CPU + Memory + IO + `target_ref` 自动闭环阶段，后续修改不得退化为只写 `cpu.weight`；`target_ref` 已可解析 cgroup/PID/container/container_id/k8s_pod 并落到目标 cgroup，后续增强重点是真实 container runtime / Kubernetes Pod 实测和更稳定的 CPU quota 指标。
