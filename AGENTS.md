...
- 当前 Resource Control 已进入 CPU + Memory + IO + `target_ref` 自动闭环阶段，后续修改不得退化为只写 `cpu.weight`；`target_ref` 已可解析 cgroup/PID/container/container_id/k8s_pod 并落到目标 cgroup，`container_id`、runtime container name 和 `k8s_pod` 名称解析已具备 121/122 runtime target 集成测试证据；CPU quota 已具备 `cpu.stat usage_usec/nr_throttled/throttled_usec` 双机效果证据；Redis + background CPU quota Compare/Sweep Benchmark 已用 `default_noisy / eulerpilot_no_quota / eulerpilot_quota` 和多档 `cpu.max` 拆分 Agent 放置影响和 quota 影响，当前只作为后台限额效果、throttling、profile 调参和业务指标边界证据，不写成 Redis 性能提升结论；跨机保守 profile 口径优先看 121/122 同时成立的边界，不得只用单机最优值推广；后续增强重点是真实容器运行时/Kubernetes lab Pod 现场演示和 Nginx 多 workload quota 调参。
