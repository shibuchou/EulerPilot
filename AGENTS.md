...
- 当前 Resource Control 已进入 CPU + Memory + IO + `target_ref` 自动闭环阶段，后续修改不得退化为只写 `cpu.weight`；`target_ref` 已可解析 cgroup/PID/container/container_id/k8s_pod 并落到目标 cgroup，`container_id`、runtime container name 和 `k8s_pod` 名称解析已具备 121/122 runtime target 集成测试证据；CPU quota 已具备 `cpu.stat usage_usec/nr_throttled/throttled_usec` 双机效果证据；Redis + background CPU quota Compare Benchmark 已用 `default_noisy / eulerpilot_no_quota / eulerpilot_quota` 三阶段拆分 Agent 放置影响和 quota 影响，当前只作为后台限额效果、throttling 和业务指标边界证据，不写成 Redis 性能提升结论；后续增强重点是真实容器运行时/Kubernetes lab Pod 现场演示和更接近业务场景的多 workload quota 调参。
