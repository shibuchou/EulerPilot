...
- 当前 Resource Control 已进入 CPU + Memory + IO + `target_ref` 自动闭环阶段，后续修改不得退化为只写 `cpu.weight`；`target_ref` 已可解析 cgroup/PID/container/container_id/k8s_pod 并落到目标 cgroup，`container_id`、runtime container name 和 `k8s_pod` 名称解析已具备 121/122 runtime target 集成测试证据；CPU quota 已具备 `cpu.stat usage_usec/nr_throttled/throttled_usec` 双机效果证据；Redis/Nginx + background CPU quota Sweep Benchmark 已用 `default_noisy / eulerpilot_no_quota / eulerpilot_quota` 和多档 `cpu.max` 拆分 Agent 放置影响和 quota 影响，Mixed Redis+Nginx sweep 已进一步验证多前台业务同时运行时的 profile 选择边界，Mixed multi-resource benchmark 已验证 `cpu.max + cpuset.cpus + memory.low/high` 组合写入、审计和 rollback；当前只作为后台限额效果、throttling、profile 调参和业务指标边界证据，不写成业务性能提升结论；跨机保守 profile 口径优先看 121/122 同时成立的边界，不得只用单机最优值推广；当前 Redis 默认演示 profile 保守使用 `quota_10`，Nginx 可演示候选为 `quota_05`，Mixed 场景需按多前台最低保留率选择 profile，121 为 `quota_20`，122 为 `quota_50`，多资源组合 profile 当前两端均推荐 `multi_quota50`；真实容器运行时/Kubernetes lab Pod 现场演示入口已补齐，当前 121/122 因缺 docker/podman/kubectl 记录为 blocked；后续增强重点是在补齐 runtime、镜像与 `eulerpilot-lab` demo Pod 后把该入口转为 pass，并继续推进跨 Skill 联动。
