...
当前 `bpf/security_policy_demo.bpf.c` 的强制控制已经从 BPF 硬编码路径切到用户态填充的 `target_map`。当前 BPF map 最多支持 8 组文件路径、文件访问权限、精确执行路径、执行路径前缀、IPv4 socket endpoint、scope-only ptrace/setuid/setgid cgroup target 和 capability target；默认 demo 配置仍指向：
...
- BPF 侧加入最小 config map 和 event ringbuf。（已完成 demo target、LSM file_open、LSM bprm_check_security、LSM socket_connect、LSM ptrace_traceme、LSM capable、LSM task_fix_setuid、LSM task_fix_setgid、execve/openat/connect/ptrace tracepoint 事件）
- BPF 侧加入动态 map：target map、path rule map、exec rule map、control map。（已完成最多 8 组 path/path_prefix/file_access/exec/exec_prefix/socket/ptrace/capability/setuid/setgid/cgroup scope `target_map`，用户态从 YAML target path、path_prefix、file_access、exec path、exec_prefix、dst_ip/dst_port、capability、可选 cgroup_path、`type: pid`、`type: container_id`、`type: container` 和 `type: k8s_pod` 解析结果填充；LSM file/bprm/socket/ptrace/capable/credential blocked 事件已支持规则级标识）
- 最低覆盖 `execve/openat/connect/ptrace` syscall tracing，以及 `file_open`、`bprm_check_security`、`socket_connect`、`ptrace_traceme`、`capable`、`task_fix_setuid` 和 `task_fix_setgid` 七类 BPF LSM enforce。（已完成四类 syscall 观测、`file_open` path/file_access enforce、`bprm_check_security` target_map exec path/exec_prefix enforce、`socket_connect` scoped IPv4 endpoint enforce、`ptrace_traceme` scoped cgroup enforce、`capable` scoped capability enforce、`task_fix_setuid` scoped setuid credential enforce 和 `task_fix_setgid` scoped setgid credential enforce）
