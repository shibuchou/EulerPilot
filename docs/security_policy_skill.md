...
本文面向比赛评委和后续 Agent，说明 EulerPilot Security Agent 的能力定位、当前完成度、参考代码复用边界和最小验收入口。当前仓库已经注册正式 `security_policy` Skill，并保留 `security_policy_demo` 兼容入口。正式入口已支持 YAML v2 `targets + rules + target_ref`，`audit` 与 `enforce` 都会 attach 最小 BPF LSM 程序并消费 ringbuf 命中事件；同时 audit 路径已补入 `sys_enter_execve`、`sys_enter_openat`、`sys_enter_connect` 与 `sys_enter_ptrace` tracepoint 观测事件。用户态已从最多 8 条 `rules.*.target_ref` 解析 `targets.<target_ref>.path`、`exec_path`、可选 `cgroup_path`、`type: pid` 自动解析出的 cgroup scope、`type: container_id` 通过 `cgroup_root` 扫描解析出的 cgroup scope、`type: container` 通过 runtime CLI 解析出的 container ID/cgroup scope，以及 `type: k8s_pod` 通过 `kubectl` 查询 Pod UID 后解析出的 cgroup scope，再写入 `BPF_ARRAY target_map`；BPF 不再硬编码 demo secret 或 demo exec 路径。2026-06-22 已补真实 `lsm/socket_connect` IPv4 `dst_ip + dst_port + cgroup_id` 阻断，socket blocked 事件会输出 endpoint 证据。`audit` 模式通过 map 配置 `enforce=0`，只记录命中、不阻断；`enforce` 模式通过 `enforce=1` 对当前 YAML 声明目标执行拒绝，并在 Agent 退出时通过 BPF link fd 自动 detach。
...
当前 `bpf/security_policy_demo.bpf.c` 的强制控制已经从 BPF 硬编码路径切到用户态填充的 `target_map`。当前 BPF map 最多支持 8 组文件路径、执行路径和 IPv4 socket endpoint；默认 demo 配置仍指向：
...
10. 创建带 container ID 字符串的临时 cgroup，并使用 `type: container_id` target 验证用户态能在限定 `cgroup_root` 下解析 cgroup scope，且 scope 外访问成功、scope 内访问拒绝。
11. 创建 fake `crictl` 命令和带 container ID 的临时 cgroup，并使用 `type: container` + `container_name` 验证 runtime CLI 解析路径。
12. 创建 fake `kubectl` 命令和带 Pod UID 的临时 cgroup，并使用 `type: k8s_pod` + `namespace/pod_name` 验证 Pod 名称解析路径。
13. 等 Agent 正常退出，确认目标文件、demo 可执行文件和 socket connect 恢复可访问，并调用 cleanup 验证无 demo 残留。

验收输出应包含 `PASS: security_policy audit mode writes file, bprm and four syscall hit events`、`PASS: security_policy enforce mode writes blocked file and bprm hit events`、`PASS: target file is denied while security_policy enforce is active`、`PASS: exec target is denied while security_policy enforce is active`、`PASS: security_policy target_map reports rule-specific dynamic YAML file and exec hits`、`PASS: security_policy cgroup scoped target only blocks inside target cgroup`、`PASS: security_policy lsm_socket_connect blocks scoped IPv4 target and reports endpoint evidence`、`PASS: security_policy pid target resolves to cgroup scoped enforcement`、`PASS: security_policy container_id target resolves to cgroup scoped enforcement`、`PASS: security_policy container runtime name target resolves to cgroup scoped enforcement`、`PASS: security_policy k8s pod name target resolves to cgroup scoped enforcement` 和 rollback 恢复类 PASS。
...
- BPF 侧加入动态 map：target map、path rule map、exec rule map、control map。（已完成最多 8 组 path/exec/socket/cgroup scope `target_map`，用户态从 YAML target path、exec path、dst_ip/dst_port、可选 cgroup_path、`type: pid`、`type: container_id`、`type: container` 和 `type: k8s_pod` 解析结果填充；LSM file/bprm/socket blocked 事件已支持规则级标识）
- 最低覆盖 `execve/openat/connect/ptrace` syscall tracing，以及 `file_open`、`bprm_check_security` 和 `socket_connect` 三类 BPF LSM enforce。（已完成四类 syscall 观测、`file_open` 最小 enforce、`bprm_check_security` target_map exec path enforce 和 `socket_connect` scoped IPv4 endpoint enforce）
...
当前可验收：正式 `security_policy` 注册名、YAML v2 path/exec_path/dst_ip/dst_port/cgroup_path/pid/container_id/container/k8s_pod target、用户态向 BPF `target_map` 填充最多 8 组文件路径、执行路径、socket endpoint 和可选 cgroup id、audit 不阻断并写 BPF hit event、audit 事件覆盖 `lsm_file_open/lsm_bprm_check_security/sys_enter_execve/sys_enter_openat/sys_enter_connect/sys_enter_ptrace`、enforce BPF LSM attach、目标文件拒绝、demo 执行文件拒绝、scoped IPv4 socket connect 拒绝、双动态 `/tmp` 目标验证、LSM blocked hit event 可映射到单条 YAML 规则、显式 cgroup scope 内阻断且 scope 外允许、PID target 自动解析到 cgroup scope、container_id target 在限定 cgroup tree 下解析到 cgroup scope、container runtime name target 通过 fake `crictl` 验证、k8s pod name target 通过 fake `kubectl` + Pod UID 验证、Agent 退出恢复、cleanup 无残留。121 最新结果目录为 `results/security_policy/integration-20260622-105820`；122 最新结果目录为 `results/security_policy/integration-20260622-110120`。121 最新质量门禁为 `reports/final_quality_gate_20260622_security_socket_connect.log`。

下一阶段正式验收：在已完成 `lsm/socket_connect` 的基础上，继续补 bprm 前缀/可写目录执行异常、file_open 权限维度、ptrace LSM、capable 等安全规则。所有新增 hook 都必须复用当前 target/cgroup scope，不要重新实现 Pod/container 解析。
