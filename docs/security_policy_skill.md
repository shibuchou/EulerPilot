...
当前 `bpf/security_policy_demo.bpf.c` 的强制控制已经从 BPF 硬编码路径切到用户态填充的 `target_map`。当前 BPF map 最多支持 8 组文件路径、文件访问权限、精确执行路径、执行路径前缀和 IPv4 socket endpoint；默认 demo 配置仍指向：
...
当前 BPF demo 已输出 ringbuf 命中事件；用户态 `security_policy` 消费事件并写入 `reports/events/security_policy.jsonl`。当前事件已区分 `event_hook=lsm_file_open`、`event_hook=lsm_bprm_check_security`、`event_hook=lsm_socket_connect`、`event_hook=sys_enter_execve`、`event_hook=sys_enter_openat`、`event_hook=sys_enter_connect` 和 `event_hook=sys_enter_ptrace`。LSM file/bprm/socket 阻断事件已携带 BPF `target_index`，用户态会映射回 YAML 中的单条 `rule_id` 和 `target_ref`；当 target 配置 `cgroup_path`、`type: pid`、`type: container_id`、`type: container` 或 `type: k8s_pod` 时，blocked 事件还会输出 `cgroup_id/cgroup_path`；socket blocked 事件还会输出 `dst_ip/dst_port/protocol`，exec_prefix blocked 事件还会输出 `exec_prefix`，file_open 权限维度 blocked 事件还会输出 `file_access/file_flags`。tracepoint 观测事件不参与规则匹配，统一写 `target_index=unknown` 并保留合并规则上下文。最小验收以“audit 不阻断且存在 `operation=hit/result=observed`、audit 事件覆盖 file_open、bprm_check_security 与四类 syscall tracepoint、enforce attach 后 target_map 中的目标文件、demo 可执行文件、scoped socket endpoint、scoped writable-dir exec_prefix 和 scoped file_access 写打开被拒绝且 blocked 事件带上具体规则、显式 cgroup target、pid target、container_id target、runtime container name target 和 k8s pod name target 都只在解析出的目标 cgroup 内阻断、Agent 退出后恢复访问、无 BPF link/pin 残留”为准。下一步成品化重点转向更多 LSM hook 和异常规则。
...
- BPF 侧加入最小 config map 和 event ringbuf。（已完成 demo target、LSM file_open、LSM bprm_check_security、LSM socket_connect、execve/openat/connect/ptrace tracepoint 事件）
- BPF 侧加入动态 map：target map、path rule map、exec rule map、control map。（已完成最多 8 组 path/file_access/exec/exec_prefix/socket/cgroup scope `target_map`，用户态从 YAML target path、file_access、exec path、exec_prefix、dst_ip/dst_port、可选 cgroup_path、`type: pid`、`type: container_id`、`type: container` 和 `type: k8s_pod` 解析结果填充；LSM file/bprm/socket blocked 事件已支持规则级标识）
- 最低覆盖 `execve/openat/connect/ptrace` syscall tracing，以及 `file_open`、`bprm_check_security` 和 `socket_connect` 三类 BPF LSM enforce。（已完成四类 syscall 观测、`file_open` path/file_access enforce、`bprm_check_security` target_map exec path/exec_prefix enforce 和 `socket_connect` scoped IPv4 endpoint enforce）
...
当前可验收：正式 `security_policy` 注册名、YAML v2 path/file_access/exec_path/exec_prefix/dst_ip/dst_port/cgroup_path/pid/container_id/container/k8s_pod target、用户态向 BPF `target_map` 填充最多 8 组文件路径、文件访问权限、执行路径、执行路径前缀、socket endpoint 和可选 cgroup id、audit 不阻断并写 BPF hit event、audit 事件覆盖 `lsm_file_open/lsm_bprm_check_security/sys_enter_execve/sys_enter_openat/sys_enter_connect/sys_enter_ptrace`、enforce BPF LSM attach、目标文件拒绝、demo 执行文件拒绝、scoped IPv4 socket connect 拒绝、scoped writable-dir exec_prefix 执行拒绝、scoped file_access 写打开拒绝且读打开放行、双动态 `/tmp` 目标验证、LSM blocked hit event 可映射到单条 YAML 规则、显式 cgroup scope 内阻断且 scope 外允许、PID target 自动解析到 cgroup scope、container_id target 在限定 cgroup tree 下解析到 cgroup scope、container runtime name target 通过 fake `crictl` 验证、k8s pod name target 通过 fake `kubectl` + Pod UID 验证、Agent 退出恢复、cleanup 无残留。121 最新结果目录为 `results/security_policy/integration-20260622-195342`；122 最新结果目录为 `results/security_policy/integration-20260622-195627`。121 最新质量门禁为 `reports/final_quality_gate_20260622_security_file_access.log`。

下一阶段正式验收：在已完成 `lsm/socket_connect`、bprm `exec_prefix` 和 file_open `file_access` 的基础上，继续补 ptrace LSM、capable、只读目录保护和异常行为规则。所有新增 hook 都必须复用当前 target/cgroup scope，不要重新实现 Pod/container 解析。
