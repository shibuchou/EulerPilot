...
本文面向比赛评委和后续 Agent，说明 EulerPilot Security Agent 的能力定位、当前完成度、参考代码复用边界和最小验收入口。当前仓库已经注册正式 `security_policy` Skill，并保留 `security_policy_demo` 兼容入口。正式入口已支持 YAML v2 `targets + rules + target_ref`，`audit` 与 `enforce` 都会 attach 最小 BPF LSM 程序并消费 ringbuf 命中事件；同时 audit 路径已补入 `sys_enter_execve`、`sys_enter_openat`、`sys_enter_connect` 与 `sys_enter_ptrace` tracepoint 观测事件。`audit` 模式通过 map 配置 `enforce=0`，只记录命中、不阻断；`enforce` 模式通过 `enforce=1` 对固定 demo secret 文件执行拒绝，并在 Agent 退出时通过 BPF link fd 自动 detach。
...
当前 `bpf/security_policy_demo.bpf.c` 的强制控制只实现了固定路径：

```text
/root/EulerPilot/demo/security_policy_demo/secret.txt
```

它已经有最小 `policy_map` 和 ringbuf 命中事件，但尚未支持动态 path/process/container target map。当前 `mode` 已由 BPF map 生效：`audit` attach BPF 但返回 allow；`enforce` attach BPF LSM 并触发固定路径拒绝。当前 tracepoint 观测已覆盖 `sys_enter_execve`、`sys_enter_openat`、`sys_enter_connect` 与 `sys_enter_ptrace`，并在 BPF 侧过滤 Agent 自身写审计日志产生的递归事件；这些 syscall 事件目前只做观测和证据输出，不参与 enforce。文档和验收时应把当前阶段称为正式 `security_policy` BPF LSM + 四类 syscall tracing 最小闭环，而不是完整 SecurityPolicySkill 成品。
...
当前 BPF demo 已输出 ringbuf 命中事件；用户态 `security_policy` 消费事件并写入 `reports/events/security_policy.jsonl`。当前事件已区分 `event_hook=lsm_file_open`、`event_hook=sys_enter_execve`、`event_hook=sys_enter_openat`、`event_hook=sys_enter_connect` 和 `event_hook=sys_enter_ptrace`。最小验收以“audit 不阻断且存在 `operation=hit/result=observed`、audit 事件覆盖 LSM file_open 与四类 syscall tracepoint、enforce attach 后目标文件被拒绝且存在 `operation=hit/result=blocked`、Agent 退出后恢复访问、无 BPF link/pin 残留”为准。下一步成品化时需要把 ringbuf 事件从固定路径 demo 扩展到动态规则命中，并继续补齐 bprm 类 hook 和容器级 target 绑定。
...
4. 使用临时 audit 配置启用正式 `security_policy`，确认目标文件仍可读且写入 BPF ringbuf 命中事件，并覆盖 `lsm_file_open`、`sys_enter_execve`、`sys_enter_openat`、`sys_enter_connect`、`sys_enter_ptrace`。
...
验收输出应包含 `PASS: security_policy audit mode writes LSM and four syscall hit events`、`PASS: security_policy enforce mode writes blocked BPF hit event`、`PASS: target file is denied while security_policy enforce is active` 和 `PASS: target file is readable after agent rollback`。
...
- BPF 侧加入最小 config map 和 event ringbuf。（已完成固定 demo path、LSM file_open、execve/openat/connect/ptrace tracepoint 事件）
- BPF 侧加入动态 map：target map、path rule map、exec rule map、control map。
- 最低覆盖 `execve/openat/connect/ptrace` syscall tracing，以及 `file_open` 和 `bprm_check_security` 两类 BPF LSM enforce。（已完成四类 syscall 观测和 `file_open` 最小 enforce；`bprm_check_security` 待补）
...
当前可验收：正式 `security_policy` 注册名、YAML v2 path target、audit 不阻断并写 BPF hit event、audit 事件覆盖 `lsm_file_open/sys_enter_execve/sys_enter_openat/sys_enter_connect/sys_enter_ptrace`、enforce BPF LSM attach、目标文件拒绝、blocked hit event、Agent 退出恢复、cleanup 无残留。
