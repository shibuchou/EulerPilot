...
5.2. 使用 `test_security_policy_credential_anomaly.sh` 启用 scoped credential anomaly 配置，触发目标 cgroup 内多次 `setuid(65534)`，确认 `security_policy_events.credential-anomaly.jsonl` 包含 `credential_churn`，并携带 `credential_stage`、`uid` 或 `cred_gfp` 等生命周期字段。
5.3. 使用 `test_security_policy_credential_deep_hooks.sh` 启用 scoped `lsm_cred_alloc_blank`、`lsm_cred_transfer`、`lsm_cred_prepare` 和 `lsm_task_fix_setuid`，确认 Agent 可 attach deep hooks，`hook_type` rule mapping 无误，并在 `deep_hook_status.txt` 记录 deep hook hit 计数。普通用户态触发未稳定命中 `cred_alloc_blank/cred_transfer` 时，结果应写 `deep_hook_runtime_note=no-userland-hit-observed`，不把 hit=0 写成失败。
