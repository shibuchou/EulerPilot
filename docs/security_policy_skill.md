...
16. 创建带 container ID 字符串的临时 cgroup，并使用 `type: container_id` target 验证用户态能在限定 `cgroup_root` 下解析 cgroup scope，且 scope 外访问成功、scope 内访问拒绝。
17. 创建 fake `crictl` 命令和带 container ID 的临时 cgroup，并使用 `type: container` + `container_name` 验证 runtime CLI 解析路径。
18. 创建 fake `kubectl` 命令和带 Pod UID 的临时 cgroup，并使用 `type: k8s_pod` + `namespace/pod_name` 验证 Pod 名称解析路径。
19. 等 Agent 正常退出，确认目标文件、demo 可执行文件、exec_prefix 目标、file_access 目标、path_prefix 目标、ptrace_traceme、capable 和 socket connect 恢复可访问，并调用 cleanup 验证无 demo 残留。
