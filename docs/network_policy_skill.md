...
- XDP 审计事件已包含 `rule_ids=drop_icmp_lab,drop_tcp_probe_lab,drop_udp_probe_lab`、`rule_count=3` 与 `target_ref=lab_xdp_veth`；rollback 事件包含每条规则的 per-rule drop/byte 统计；真实 Pod XDP 事件包含 `target_ref=lab_pod`、真实 `ifname/ifindex` 和 `drop_count`。
...
1. 为 TC QoS 增加多规则验证。
2. 将 real Pod host veth XDP 扩展到 UDP 和更多报文特征。
