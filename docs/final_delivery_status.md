...
- `192.168.1.121:/root/EulerPilot`：SP3 初代迭代仓库和历史回归对照。
...
- Redis/Nginx RUNS=5、Redis pressure gradient、static-vs-Agent 修复后 RUNS=5、throughput-first、mixed-adaptive、agent overhead。
- Web Console v1：Evidence-first + 白名单 Demo + 旁路展示控制台。
- final evidence strict：40 条核心证据，缺失 0，警告 0。
- final quality gate：22/22 P0、100 轮 smoke、5 轮 doctor 通过。

## 当前最重要的结果目录

- Redis SP4：`/root/EulerPilot/results/final/redis-scx-compare-20260708-150702`
- Nginx SP4：`/root/EulerPilot/results/final/nginx-scx-compare-20260708-152602`
- Redis pressure gradient：`/root/EulerPilot/results/final/redis-pressure-gradient-20260708-153811`
- Redis static vs Agent dynamic：`/root/EulerPilot/results/final/redis-static-vs-agent-20260720-150342`
