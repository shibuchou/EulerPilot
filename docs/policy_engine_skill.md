...
v3.2 已完成真实 Kubernetes Pod 联动：同一个 `target_ref=lab_pod(type=k8s_pod)` 可在 `policy_engine` 内按动作类型解析为 Pod cgroup 或 Pod host veth，并保留 `target_type=k8s_pod`、`resolved_target_type=cgroup|netdev`、Pod namespace/name/UID、`transaction_id` 和 ActionJournal 证据。服务联动 Security anomaly 已补齐 `burst_connect`、`burst_openat_sensitive` 和 `capability_abuse` 双机证据，结果目录为 `results/security_policy/anomaly-rules-20260703-121-v4` 和 `results/security_policy/anomaly-rules-20260703-122-v2`。Security credential 生命周期 anomaly 已补齐 `credential_churn` 双机证据，结果目录为 `results/security_policy/credential-anomaly-20260703-121-v4` 和 `results/security_policy/credential-anomaly-20260703-122-v4`；credential deep hook 评估已补齐 `cred_alloc_blank/cred_transfer` scoped attach 与 `hook_type` 映射证据，结果目录为 `results/security_policy/credential-deep-hooks-20260703-121-v2` 和 `results/security_policy/credential-deep-hooks-20260703-122-v2`。Security anomaly 进程过滤与组合 scope 过滤已补齐双机证据，结果目录为 `results/security_policy/anomaly-process-filter-20260703-121-v1`、`results/security_policy/anomaly-process-filter-20260703-122-v1`、`results/security_policy/anomaly-combo-scope-20260703-121-v1` 和 `results/security_policy/anomaly-combo-scope-20260703-122-v1`。isolated-veth XDP 已补齐 ICMP/TCP/UDP + UDP tuple 四规则与 per-rule 字段统计，结果目录为 `results/network_policy/xdp-20260703-121-fields-v1` 和 `results/network_policy/xdp-20260703-122-fields-v1`；real Pod host veth XDP 也已补齐 ICMP/TCP/UDP + UDP tuple 四规则，结果目录为 `results/network_policy/real-pod-veth-xdp-20260703-k3s-121-tuple-v1` 和 `results/network_policy/real-pod-veth-xdp-20260703-k3s-122-tuple-v1`。后续争奖增强重点转向答辩材料冻结、现场演示压测和 SP4 发布后平台复核。
