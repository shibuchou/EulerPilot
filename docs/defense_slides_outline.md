...
## 第 7 页：Redis formal artifact 结果与历史对照
...
说明：formal artifact RUNS=10 是正式收益入口；旧图表只作为历史对照和修复前问题说明。
...
4. 业务结果已写入 formal artifact 结果目录，并保留历史候选目录作为过程追溯
...
4. 42 条 evidence compact + Web Console + v6 preflight 已完成；最终 release gate 需绑定同一 candidate SHA 与 formal artifact
