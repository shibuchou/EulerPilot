...
阶段 F：Kubernetes 与跨 Agent 联动，状态：`已开始；Security anomaly -> Policy Engine -> Resource Control 降级最小闭环已完成`
...
- Policy Engine 已完成第一条跨 Skill 链路：监听 `security_policy` 的 `burst_execve` anomaly 事件，经白名单动作写入显式 cgroup 的 `cpu.max` 与 `memory.high`，并输出 `policy_engine.jsonl` 与 `ActionJournal`，Agent stop 后恢复旧值。
...
| F. Kubernetes 与跨 Agent 联动 | 已开始 / Security -> Resource 最小闭环已完成 | 新增正式 `policy_engine` Skill；当前完成 `security_policy` 的 `burst_execve` anomaly 到 Resource Control cgroup 降级链路，写入 `cpu.max=10000 100000` 与 `memory.high=1048576`，事件、ActionJournal 和 rollback 均已在 121/122 通过；后续继续扩展 Network QoS 联动与更多 anomaly 规则 | `docs/policy_engine_skill.md`、`agent/skills/policy_engine/README.md`、`tests/integration/test_policy_engine_security_resource.sh`、`results/policy_engine/security-resource-20260629-163949`、`results/policy_engine/security-resource-20260629-164135` |
