...
- 验证与交付基线已调整：`192.168.1.123:/root/EulerPilot` 的 openEuler 24.03 LTS SP4 仓库作为核心验证和最终交付验证仓库；`192.168.1.121:/root/EulerPilot` 降级为 SP3 历史验证和回归对照仓库。
...
- SP4 争奖性能证据已补强：`results/final/throughput-first-20260720-165544` 验证显式 `EULERPILOT_THROUGHPUT_FIRST=1` 下 batch/throughput 路径命中；`results/final/mixed-adaptive-20260720-170840` 验证压力出现、PSI gate ACTIVE、调度更新、recovery 和 rollback；`results/final/agent-overhead-20260720-170415` 给出 Agent CPU/RSS 控制面开销。
...
- 121 历史质量门禁：`reports/final_quality_gate_20260706-quality-121.log`，22/22 P0、100 轮 Agent smoke、5 轮 doctor 均通过；新增 C++ unit tests。当前最终入口以 SP4 `reports/final_quality_gate_20260720-stage3-performance.log` 为准。
...
- SP4 Redis sched_ext RUNS=5 多轮对照已通过，结果目录：`results/final/redis-scx-compare-20260708-150702`。
- SP4 Nginx sched_ext RUNS=5 多轮对照已通过，结果目录：`results/final/nginx-scx-compare-20260708-152602`。
- SP4 Redis 压力递增梯度已通过，结果目录：`results/final/redis-pressure-gradient-20260708-153811`，覆盖 `STRESS_WORKERS=0/1/2/4/8`。
- SP4 Redis 人工静态 vs Agent 动态对比已用修复脚本重跑：旧目录 `redis-static-vs-agent-20260708-162543` 因只移动 stress-ng 父 PID 已撤下；新目录 `results/final/redis-static-vs-agent-20260720-150342` 在目标 cgroup 内启动 stress-ng 及其 worker，manual_static 与 agent_dynamic 均记录 controlled PIDs 与 background cgroup procs。
- SP4 throughput-first 批处理证据已通过，结果目录：`results/final/throughput-first-20260720-165544`，`cgroup_throughput_first` 记录 throughput profile hits，`scx_throughput_first` 记录 `class_hits_batch/enqueue_batch/running_batch` 证据；该路径通过显式环境变量启用，默认 Agent 行为不变。
- SP4 mixed-adaptive 完整闭环证据已通过，结果目录：`results/final/mixed-adaptive-20260720-170840`，RUNS=3 下 pressure 阶段均触发 ACTIVE，recovery 阶段不再误报 ACTIVE，报告明确 PSI probe 只用于触发证据，不作为净性能收益结论。
- SP4 Agent 控制面开销证据已通过，结果目录：`results/final/agent-overhead-20260720-170415`，覆盖 observe-only cgroup、active cgroup 和 active sched_ext 三组 CPU/RSS 采样。
...
- Web Console v1 已在 SP4 自编译 sched_ext 内核下恢复运行：`/api/health` 正常，`/api/system` 显示 `sched_ext=true`，`/api/evidence/summary` 显示 40 条核心证据、必需缺失 0、警告 0。
...
| E. SP4/sched_ext 复核 | SP4 sched_ext 自编译内核与 RUNS=5 workload 对照已通过 | 123 已接入 openEuler 24.03 LTS SP4；发行内核默认未启用 `CONFIG_SCHED_CLASS_EXT`，当前已自编译并启动 `6.6.0-159.4.3.154.oe2403sp4.x86_64-eulerpilot-scx`，`CONFIG_SCHED_CLASS_EXT=y`、`/sys/kernel/sched_ext` 可用；新内核下 `check_sp4_env`、质量门禁、Web Console Evidence、v3.1 `security_policy -> policy_engine -> resource_control + network_qos -> rollback` repeat 10、Redis smoke、Redis PSI ACTIVE probe、Redis RUNS=5 compare、Nginx RUNS=5 compare、Redis pressure gradient、static-vs-agent、throughput-first、mixed-adaptive 和 Agent overhead 均通过 | `docs/sp4_validation_plan.md`、`reports/sp4/sp4_sched_ext_validation_20260705-211407.md`、`reports/sp4/final_quality_gate_scx_workload_20260706-1214.log`、`results/policy_engine/security-network-resource-20260705-211407`、`results/reports/redis-scx-smoke-20260706-092724`、`results/final/redis-scx-psi-probe-20260706-100857`、`results/final/redis-scx-compare-20260708-150702`、`results/final/nginx-scx-compare-20260708-152602`、`results/final/redis-pressure-gradient-20260708-153811`、`results/final/redis-static-vs-agent-20260720-150342`、`results/final/throughput-first-20260720-165544`、`results/final/mixed-adaptive-20260720-170840`、`results/final/agent-overhead-20260720-170415` |
...
| G. Benchmark 与冻结材料 | 已完成 | Redis/Nginx 主候选结果、Network QoS 速率误差、Resource Control quota sweep、Policy Engine 演示压测、SP4 RUNS=5 复核、Redis pressure gradient、static-vs-agent、throughput-first、mixed-adaptive、Agent overhead、SP4/K8s 旁路验证和 40 条 final evidence compact 已冻结；后续只追加现场彩排截图/录屏，不覆盖主结论 | `docs/stage_g_benchmark_freeze.md`、`configs/final_evidence_manifest.json`、`reports/final_evidence_compact.md`、`results/final/redis-scx-compare-20260708-150702`、`results/final/nginx-scx-compare-20260708-152602`、`results/final/redis-pressure-gradient-20260708-153811`、`results/final/redis-static-vs-agent-20260720-150342`、`results/final/throughput-first-20260720-165544`、`results/final/mixed-adaptive-20260720-170840`、`results/final/agent-overhead-20260720-170415`、`results/k8s/sp4-validation-20260708-023552` |
