...
- SP4 当前运行 `6.6.0-159.4.3.154.oe2403sp4.x86_64-eulerpilot-scx`，Web Console `/api/system` 显示 `cgroup_v2=true`、`psi=true`、`sched_ext=true`、`btf=true`。
...
- SP4 Redis sched_ext RUNS=3 多轮对照已通过，结果目录：`results/final/redis-scx-compare-20260706-115029`。
- SP4 Nginx sched_ext RUNS=3 多轮对照已通过，结果目录：`results/final/nginx-scx-compare-20260706-120547`。
...
- Web Console v1 已在 SP4 sched_ext 内核下恢复运行：`/api/health` 正常，`/api/system` 显示 `sched_ext=true`，`/api/evidence/summary` 显示 35 条核心证据、必需缺失 0、警告 0。
...
| E. SP4/sched_ext 复核 | SP4 sched_ext 自编译内核与 RUNS=3 workload 对照已通过 | 123 已接入 openEuler 24.03 LTS SP4；发行内核默认未启用 `CONFIG_SCHED_CLASS_EXT`，当前已自编译并启动 `6.6.0-159.4.3.154.oe2403sp4.x86_64-eulerpilot-scx`，`CONFIG_SCHED_CLASS_EXT=y`、`/sys/kernel/sched_ext` 可用；新内核下 `check_sp4_env`、质量门禁、Web Console Evidence、v3.1 `security_policy -> policy_engine -> resource_control + network_qos -> rollback` repeat 10、Redis smoke、Redis PSI ACTIVE probe、Redis RUNS=3 compare 和 Nginx RUNS=3 compare 均通过 | `docs/sp4_validation_plan.md`、`reports/sp4/sp4_sched_ext_validation_20260705-211407.md`、`reports/sp4/final_quality_gate_scx_workload_20260706-1214.log`、`results/policy_engine/security-network-resource-20260705-211407`、`results/reports/redis-scx-smoke-20260706-092724`、`results/final/redis-scx-psi-probe-20260706-100857`、`results/final/redis-scx-compare-20260706-115029`、`results/final/nginx-scx-compare-20260706-120547` |
...
| G. Benchmark 与冻结材料 | 已完成 | Redis/Nginx 主候选结果、Network QoS 速率误差、Resource Control quota sweep、Policy Engine 演示压测、SP4 RUNS=3 复核、SP4/K8s 旁路验证和 35 条 final evidence compact 已冻结；后续只追加现场彩排截图/录屏，不覆盖主结论 | `docs/stage_g_benchmark_freeze.md`、`configs/final_evidence_manifest.json`、`reports/final_evidence_compact.md`、`results/final/redis-scx-compare-20260706-115029`、`results/final/nginx-scx-compare-20260706-120547`、`results/k8s/sp4-validation-20260708-023552` |
