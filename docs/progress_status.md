...
- v3.2 剩余争奖增强重点转为：更多异常策略组合、进程过滤评估和最终答辩证据压缩。
...
- `TargetResolver` 已从 netdev 与 `k8s_pod` 诊断入口推进到 container name/ID、Pod UID、runtime PID、netns 和 host veth/ifindex 真实解析；真实 k3s lab Pod 已接入 TC QoS、XDP ICMP/TCP/UDP + UDP tuple 和 Policy Engine 跨 Skill 联动演示，isolated-veth 与 real Pod XDP 均已补齐 tuple 多字段匹配，后续重点是 credential 深层 hook 和证据压缩。
