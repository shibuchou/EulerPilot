...
## v6 封版收口状态（2026-07-26）

- 当前进入复审问题全量收口阶段，不再扩展新功能；正式性能实验必须绑定 `tested_code_commit`、formal `artifact_id` 和仓库外 runs 目录。
- 交付矩阵保持：SP4/123 是主开发、sched_ext/scx、Web、性能和 release candidate 环境；SP3/121 是比赛要求的强制兼容交付环境；122 只保留历史 OLK/sched_ext 对照。
- 旧 SP4 Redis/Nginx、pressure gradient、static-vs-Agent、throughput-first、mixed-adaptive、Agent overhead 结果均通过 `evidence/evidence_status_overrides.json` 降级为 provisional 或 invalid historical。旧原始结果目录保持只读，不改写哈希。
- 已修复并验证：Security fail-closed scope/protocol、cpuset release 默认关闭、PolicyEngine parser/transaction-local 状态、ResourceControl verify-fail rollback、配置字段消费审计、SCX legacy fallback 清理、benchmark default baseline/randomized block 语义。
- 2026-07-26 在 SP4 closeout 工作树运行缩短版 preflight 质量门：`29/29 P0` 通过，已纳入配置消费审计、benchmark baseline/randomized block、PolicyEngine 事务模型、ResourceControl rollback 模型、SCX loader ownership、Security fail-closed 和 evidence validate-run 正负 fixture；这不是最终 release gate。
- Candidate-bound gates 已在同一 SHA `7a99d87048f4f2040377354bfe0ce21401664642` 上通过 Portable CI、SP4 Candidate Gate 和 SP3 Candidate Gate；Formal Artifact Gate 已绑定 artifact `ef1baebec7ac138acd0eb1a59fc3880ca550330ef89f87615e8579a0ef264240` 通过。
- 修正基线后的正式 RUNS=10 套件已完成并通过 `--validate-suite`：Redis、Nginx、throughput-first、mixed-adaptive 和 Agent overhead。阶段报告为 `reports/v6_formal_experiment_suite_20260726.md`；原始输出位于仓库外 `/root/eulerpilot-runs/7a99d87048f4f2040377354bfe0ce21401664642/formal-experiments/20260726-162050`。
- SP4 final gate 已通过：`reports/final_quality_gate_20260726-v6-sp4-final.log`，29/29 P0、100 轮 Agent smoke、5 轮 doctor-safe 均通过。
- SP3 final compatibility gate 已通过：`reports/final_quality_gate_20260726-v6-sp3-final.log`，10/10 通过，并生成 `reports/gates/sp3_capability_matrix_20260726-v6-final.json`。
- 下一步仍需完成：将 formal 原始结果打包到 release bundle、生成 RUNS=10 最终图表/答辩材料、形成最终 release metadata，检查 `tested_code_commit..release_candidate_commit` 核心等价，并在用户确认后决定是否创建 tag/release。

## SP4/Kubernetes 历史验证批次（2026-07-24）
...
- Kubernetes master 已完成只读盘点：context 为 `kubernetes-admin@kubernetes`，节点 `k8s-master/k8s-worker1/k8s-worker2` 均为 `Ready`，RuntimeClass `kata` 存在；既有工具链包括 vArmor、Kata/KataLSM、Kafka、Elasticsearch/Kibana、ebpf nodeport/service、kube-flannel 和 chaos-mesh。
- 本次验证前已有 `ebpf-service-system/ebpf-clusterip-agent-h8lw2` 与 `ebpf-service-test/clusterip-client-cnf74` 为 `ImagePullBackOff`，已在报告中记录为既有基线噪声。
- Kubernetes 最小写入验证已通过：只创建 `eulerpilot-sp4-validation` namespace、`eulerpilot-k8s-smoke` Deployment 和 ClusterIP Service，全部带 `app.kubernetes.io/part-of=eulerpilot-validation` 与 `eulerpilot.io/owner=web-console` 标签，并设置 `cpu=10m/50m`、`memory=16Mi/64Mi` requests/limits。
- K8s lab Pod 本地健康检查与 Service 健康检查均返回 `ok`；验证结束后已删除 namespace，两个 EulerPilot label selector cleanup 复查均返回 `No resources found`。
- SP4 Web Console 白名单动作已通过：`check_env`、`list_skills`、`status_json`、`validate_default_config`、`doctor_skills`、`demo_offline`、`demo_cleanup` 均为 `succeeded`。
- SP4 历史质量门禁曾通过：`scripts/final_quality_gate.sh` 22/22 P0、100 轮 Agent smoke 和 5 轮 doctor，日志为 `reports/final_quality_gate_20260720-stage3-performance.log`。v6 后最终 release gate 需要重跑。
- SP4 frozen-code 历史性能证据已保留：Redis/Nginx RUNS=10、static-vs-Agent RUNS=10、throughput-first RUNS=10、mixed-adaptive RUNS=10、Agent overhead RUNS=10 与 pressure gradient RUNS=3 均位于 `results/final/*20260724-tested-2541464*`，但当前均不是 final positive evidence。
...
- 真实 Pod host veth XDP ICMP/TCP/UDP + UDP tuple 已在 121/122 通过：121 `results/network_policy/real-pod-veth-xdp-20260703-k3s-121-tuple-v1`，122 `results/network_policy/real-pod-veth-xdp-20260703-k3s-122-tuple-v1`；验证 `network_xdp` 解析 lab Pod host veth，挂 generic XDP，Pod netns 到 cni bridge 的 ICMP、TCP:19092、UDP:19093 和 `pod_ip:39094 -> bridge_ip:19094` UDP tuple 流量被 `XDP_DROP` 命中，退出后 XDP detached 且连通性恢复。两端当前统计一致：ICMP 1、TCP 4、UDP 8、UDP tuple 8，总 drop 21。
...
- Security anomaly 进程过滤已在 121/122 通过：121 `results/security_policy/anomaly-process-filter-20260703-121-v1`，122 `results/security_policy/anomaly-process-filter-20260703-122-v1`；验证同样的 `/etc` openat burst 只触发 `comm_prefix=python` 的 `burst_openat_python_prefix`，`comm=nohit-proc` 的负向规则不输出 anomaly。
- Security anomaly 组合 scope 过滤已在 121/122 通过：121 `results/security_policy/anomaly-combo-scope-20260703-121-v1`，122 `results/security_policy/anomaly-combo-scope-20260703-122-v1`；验证 scope 外同样的 Python `/etc` open burst 不触发，进入目标 cgroup 后同一条规则同时匹配 `target_ref`、`path_prefix` 与 `comm_prefix` 才输出 anomaly，并携带 `target_ref_filter/cgroup_id` 证据。
...
- 121 SP3 强制兼容历史质量门禁：`reports/final_quality_gate_20260706-quality-121.log`，22/22 P0、100 轮 Agent smoke、5 轮 doctor 均通过；新增 C++ unit tests。v6 final release gate 仍需在 SP3/121 对同一 candidate SHA 重跑。
- 最终答辩证据压缩入口已完成：`configs/final_evidence_manifest.json`、`scripts/collect_final_evidence.py`、`reports/final_evidence_compact.md`、`reports/final_evidence_compact.json`。当前 `python3 scripts/collect_final_evidence.py` 覆盖 41 个核心证据条目，必需证据缺失为 0、预期警告为 8；这些警告来自旧结果降级。
- Web Console v1 已作为旁路展示控制台落地：`web_console/` 独立存在，不修改 `agent/`、`bpf/`、`sched/` 主干；支持 Evidence-first 展示、白名单 Demo、SSE 日志、`demo/lab/cleanup` 单任务锁和 SP4 loopback 部署。2026-07-26 已在 SP4 通过 `npm test`、`npm run lint`、`npm run build`，并新增 job 失败 cleanup 与 cancel 状态保持回归测试。设计文档：`docs/web_console_design.md`，安全说明：`web_console/docs/security.md`。
- SP4 平台功能复核已完成：123 已启动基于 SP4 官方源码自编译启用 sched_ext 的 `6.6.0-159.4.3.154.oe2403sp4.x86_64-eulerpilot-scx`，`CONFIG_SCHED_CLASS_EXT=y`、`/sys/kernel/sched_ext` 可用；`scripts/check_sp4_env.sh`、`make agent`、`--validate-config`、`--doctor-skills` 和历史质量门禁均通过。Redis/Nginx `RUNS=10` 结果当前为 historical/provisional，不再作为 final positive evidence。
- SP4 sched_ext 内核下已复核 v3.1 第二条跨 Skill 联动：单轮 `tests/integration/test_policy_engine_security_network_resource.sh` 通过，`--repeat 10` 也通过，结果目录为 `results/policy_engine/security-network-resource-20260705-211329` 与 `results/policy_engine/security-network-resource-20260705-211407`。
- SP4 `scx_eulerpilot` workload 验证已完成第一轮：`scripts/build_scx_eulerpilot.sh` 可在 SP4 内核源码树下构建并安装 `/usr/local/bin/scx_eulerpilot`，调度器启动后会自动 pin `class_map/gate_state_map/stats` 到 `/sys/fs/bpf/eulerpilot/scx_eulerpilot/v1/`，Agent `--backend sched_ext --active` 直接通过。
- SP4 Redis sched_ext smoke 已通过，结果目录：`results/reports/redis-scx-smoke-20260706-092724`。
- SP4 Redis PSI ACTIVE probe 已通过，`psi_gate_trace.jsonl` 出现 5 条 `next_state=ACTIVE`，结果目录：`results/final/redis-scx-psi-probe-20260706-100857`。
- SP4 Redis sched_ext compare 已通过，结果目录：`results/final/redis-scx-compare-20260706-101505`；短 compare 中 PSI 组保留 gate update 证据，ACTIVE 触发由独立 probe 目录提供。
- SP4 Nginx sched_ext compare 已通过，结果目录：`results/final/nginx-scx-compare-20260706-101928`。
- SP4 Redis sched_ext RUNS=10 historical/provisional 结果目录：`results/final/redis-scx-compare-20260724-tested-2541464-runs10`。
- SP4 Nginx sched_ext RUNS=10 historical/provisional 结果目录：`results/final/nginx-scx-compare-20260724-tested-2541464-runs10`。
- SP4 Redis 压力递增梯度 historical/provisional 结果目录：`results/final/redis-pressure-gradient-20260724-tested-2541464-runs3`，覆盖 `STRESS_WORKERS=0/1/2/4/8`。
- SP4 Redis 人工静态 vs Agent 动态对比保留为 historical/provisional：旧目录 `redis-static-vs-agent-20260708-162543` 因只移动 stress-ng 父 PID 已撤下；20260724 目录仍需等待 formal artifact 和修正 baseline 后重跑。
- SP4 throughput-first 批处理证据已标为 invalid historical，结果目录：`results/final/throughput-first-20260724-tested-2541464-runs10`；原因是 `enqueue_batch>0` 但缺少 class-aware batch dispatch 证据。
- SP4 mixed-adaptive 闭环证据已标为 invalid historical，结果目录：`results/final/mixed-adaptive-20260724-tested-2541464-runs10-lite`；原因是未证明 quiet/pressure/recovery 来自同一个 Agent instance 和单 writer trace。
- SP4 Agent 控制面开销证据保留为 historical/provisional，结果目录：`results/final/agent-overhead-20260724-tested-2541464-runs10`。
- SP4 历史质量门禁日志：`reports/final_quality_gate_20260720-stage3-performance.log`；v6 后缩短版 preflight 29/29 已通过，最终 release gate 待 candidate/formal artifact 后重跑。
- Web Console v1 已在 SP4 自编译 sched_ext 内核下恢复运行：`/api/health` 正常，`/api/system` 显示 `sched_ext=true`；当前 evidence summary 应显示 41 条核心证据、必需缺失 0、预期警告 8。
- `scripts/setup_cgroup_v2.sh` 已修复少核 VM 兼容问题：旧默认 cpuset 分组写入失败时会回退到父 cgroup 有效 CPU/MEM 集合，避免 SP4 小规格虚拟机报 `Numerical result out of range`。
- v3.2 剩余争奖增强重点转为：答辩材料冻结、现场演示压测和最终多端一致性收口。
...
阶段 C：Security Agent 正式化，状态：`BPF LSM + socket_connect + bprm exec_prefix + file_access/path_prefix + ptrace_traceme + capable + task_fix_setuid + task_fix_setgid + task_fix_setgroups + cred_prepare + cred_alloc_blank/cred_transfer scoped attach 评估 + syscall tracing + burst_execve/burst_connect/burst_openat_sensitive/capability_abuse/credential_churn anomaly + anomaly target_ref scope 过滤 + comm/comm_prefix 进程过滤 + 多目标 target_map + 规则级事件标识 + cgroup/pid/container/runtime/pod scope 最小闭环已完成`
...
- `TargetResolver` 已从 netdev 与 `k8s_pod` 诊断入口推进到 container name/ID、Pod UID、runtime PID、netns 和 host veth/ifindex 真实解析；真实 k3s lab Pod 已接入 TC QoS、XDP ICMP/TCP/UDP + UDP tuple 和 Policy Engine 跨 Skill 联动演示，isolated-veth 与 real Pod XDP 均已补齐 tuple 多字段匹配，credential deep hook scoped attach 评估与最终证据压缩也已完成。
...
| 阶段 | 状态 | 当前结论 | 主要证据 |
|------|------|----------|----------|
| A. 公共基础设施 | 已完成 | 远端 Git、项目文档规范、README 覆盖、公共控制面最小代码和现有质量门禁已完成；后续随正式 Skill 深度接入 | `README.md`、本文件、各目录 README、`docs/public_control_plane_design.md`、`reports/final_quality_gate_20260618_control_plane.log` |
| B. Network Policy | 真实 Pod host veth QoS/XDP 双机 pass，isolated-veth 与 real Pod XDP tuple 字段双机 pass | 正式 `network_policy` 注册名已落地；connect4 audit/enforce 已完成；TC QoS 最小闭环与速率误差 Benchmark 已完成；isolated-veth XDP 与 real Pod host veth XDP 均已从 ICMP/TCP/UDP 三规则扩展到协议 + 源/目的 IP + 源/目的端口 tuple 字段闭环；schema v2 `targets + rules + target_ref` 已落地；`TargetResolver` 已支持 netdev、container name/ID、Pod UID、runtime container ID/PID、netns path 和 host veth/ifindex 解析；`network_qos` 与 `network_xdp` 已可接受 `type: container` / `type: k8s_pod` target 并解析成 host veth ifname；TC 与 XDP 已在真实 k3s lab Pod host veth 上双机通过 | `docs/network_policy_skill.md`、`docs/network_pod_veth_target.md`、`tests/integration/test_network_policy.sh`、`tests/integration/test_network_qos_tc.sh`、`tests/integration/test_network_xdp.sh`、`tests/integration/test_network_xdp_real_pod_veth.sh`、`tests/integration/test_target_resolver.sh`、`tests/benchmark/test_network_qos_rate.sh` |
| C. Security Agent | 九类稳定 LSM enforce + deep credential hook scoped attach + 四类 syscall tracing + runtime anomaly + process filter + combo scope filter + 多目标 target_map + 规则级事件标识 + cgroup/pid/container/runtime/pod scope 最小闭环已完成 | 正式 `security_policy` 注册名已落地；YAML v2 target/rule、最多 8 项 BPF `target_map`、audit BPF attach 不阻断、enforce BPF LSM blocked hit、`lsm_socket_connect`、`lsm_bprm_check_security` exec_prefix、`lsm_file_open` file_access/path_prefix、`lsm_ptrace_traceme`、`lsm_capable`、`lsm_task_fix_setuid`、`lsm_task_fix_setgid`、`lsm_task_fix_setgroups`、`lsm_cred_prepare`、`lsm_cred_alloc_blank/lsm_cred_transfer` scoped attach、`burst_execve`、`burst_connect`、`burst_openat_sensitive`、`capability_abuse`、`credential_churn` 用户态异常规则、anomaly `target_ref` scope 过滤、`comm/comm_prefix` 进程过滤、规则级事件、显式 cgroup/PID/container_id/runtime container/k8s_pod scope 和 rollback 无残留已在 121/122 通过；下一步转向答辩材料冻结和现场演示压测 | `docs/security_policy_skill.md`、`agent/skills/security_policy/README.md`、`tests/integration/test_security_policy.sh`、`tests/integration/test_security_policy_anomaly_rules.sh`、`tests/integration/test_security_policy_anomaly_process_filter.sh`、`tests/integration/test_security_policy_anomaly_combo_scope.sh`、`tests/integration/test_security_policy_credential_anomaly.sh`、`tests/integration/test_security_policy_credential_deep_hooks.sh`、`demo/security_policy_demo/README.md` |
...
| E. SP4/sched_ext 复核 | SP4 sched_ext 自编译内核功能复核已完成；旧 RUNS=10 workload 结果已降级 | 123 已接入 openEuler 24.03 LTS SP4；发行内核默认未启用 `CONFIG_SCHED_CLASS_EXT`，当前已自编译并启动 `6.6.0-159.4.3.154.oe2403sp4.x86_64-eulerpilot-scx`，`CONFIG_SCHED_CLASS_EXT=y`、`/sys/kernel/sched_ext` 可用；新内核下 `check_sp4_env`、Web Console、v3.1 `security_policy -> policy_engine -> resource_control + network_qos -> rollback` repeat 10、Redis smoke 与 Redis PSI ACTIVE probe 已有历史功能证据；Redis/Nginx RUNS=10、pressure gradient、static-vs-agent、Agent overhead 为 provisional historical，throughput-first 与 mixed-adaptive 为 invalid historical | `docs/sp4_validation_plan.md`、`reports/sp4/sp4_sched_ext_validation_20260705-211407.md`、`reports/sp4/final_quality_gate_scx_workload_20260706-1214.log`、`results/policy_engine/security-network-resource-20260705-211407`、`results/reports/redis-scx-smoke-20260706-092724`、`results/final/redis-scx-psi-probe-20260706-100857`、`results/final/redis-scx-compare-20260724-tested-2541464-runs10`、`results/final/nginx-scx-compare-20260724-tested-2541464-runs10`、`results/final/redis-pressure-gradient-20260724-tested-2541464-runs3`、`results/final/redis-static-vs-agent-20260724-tested-2541464-runs10`、`results/final/throughput-first-20260724-tested-2541464-runs10`、`results/final/mixed-adaptive-20260724-tested-2541464-runs10-lite`、`results/final/agent-overhead-20260724-tested-2541464-runs10` |
...
| G. Benchmark 与冻结材料 | v6 需重做正式冻结 | Redis/Nginx 主候选结果、Network QoS 速率误差、Resource Control quota sweep、Policy Engine 演示压测、SP4/K8s 旁路验证和 41 条 evidence compact 已保留；旧 SP4 performance evidence 不再作为 final positive evidence。下一步需要 Candidate Gate、formal artifact、修正 baseline 后正式随机化实验和 `--validate-suite`，再冻结 Stage G | `docs/stage_g_benchmark_freeze.md`、`configs/final_evidence_manifest.json`、`reports/final_evidence_compact.md`、`evidence/evidence_status_overrides.json`、`results/final/redis-scx-compare-20260724-tested-2541464-runs10`、`results/final/nginx-scx-compare-20260724-tested-2541464-runs10`、`results/final/redis-pressure-gradient-20260724-tested-2541464-runs3`、`results/final/redis-static-vs-agent-20260724-tested-2541464-runs10`、`results/final/throughput-first-20260724-tested-2541464-runs10`、`results/final/mixed-adaptive-20260724-tested-2541464-runs10-lite`、`results/final/agent-overhead-20260724-tested-2541464-runs10`、`results/k8s/sp4-validation-20260708-023552` |
...
| A3 | 项目文档规范落地 | 已完成 | 已要求阶段收口同步更新文档、README 和进度看板 |
...
- `bpf/network_xdp.bpf.c` 已从单规则扩展为最多 8 条 XDP 规则，提供 generic XDP filter、聚合统计和 per-rule `pass/drop/byte` 统计输出。
- 新增构建目标 `make network-xdp`。
...
- 历史质量门禁 `reports/final_quality_gate_20260629_policy_engine.log` 曾通过 21/21 P0；当前最终质量门禁已升级为 SP4 `reports/final_quality_gate_20260720-stage3-performance.log`，22/22 P0、100 轮 Agent smoke 和 5 轮 doctor 均通过。
...
- `tests/integration/test_security_policy_anomaly_process_filter.sh` 已在 121/122 真实验证 anomaly 进程过滤；121 结果目录：`results/security_policy/anomaly-process-filter-20260703-121-v1/`，122 结果目录：`results/security_policy/anomaly-process-filter-20260703-122-v1/`。
- `tests/integration/test_security_policy_anomaly_combo_scope.sh` 已在 121/122 真实验证 `target_ref + path_prefix + comm_prefix` 组合过滤；121 结果目录：`results/security_policy/anomaly-combo-scope-20260703-121-v1/`，122 结果目录：`results/security_policy/anomaly-combo-scope-20260703-122-v1/`。
- `tests/integration/test_security_policy_credential_anomaly.sh` 已在 121/122 真实验证并回归 `credential_churn` credential 生命周期 anomaly 规则；121 结果目录：`results/security_policy/credential-anomaly-20260703-121-v4/`，122 结果目录：`results/security_policy/credential-anomaly-20260703-122-v4/`。
- `tests/integration/test_security_policy_credential_deep_hooks.sh` 已在 121/122 真实验证 `lsm_cred_alloc_blank/lsm_cred_transfer` scoped attach 与 `hook_type` 规则映射；121 结果目录：`results/security_policy/credential-deep-hooks-20260703-121-v2/`，122 结果目录：`results/security_policy/credential-deep-hooks-20260703-122-v2/`。
...
- 121 最新正式 `security_policy` anomaly process filter 证据目录：`results/security_policy/anomaly-process-filter-20260703-121-v1/`。
- 122 最新正式 `security_policy` anomaly process filter 证据目录：`results/security_policy/anomaly-process-filter-20260703-122-v1/`。
- 121 最新正式 `security_policy` anomaly combo scope 证据目录：`results/security_policy/anomaly-combo-scope-20260703-121-v1/`。
- 122 最新正式 `security_policy` anomaly combo scope 证据目录：`results/security_policy/anomaly-combo-scope-20260703-122-v1/`。
- 121 最新正式 `security_policy` credential anomaly 规则证据目录：`results/security_policy/credential-anomaly-20260703-121-v4/`。
- 122 最新正式 `security_policy` credential anomaly 规则证据目录：`results/security_policy/credential-anomaly-20260703-122-v4/`。
- 121 最新正式 `security_policy` credential deep hook 评估证据目录：`results/security_policy/credential-deep-hooks-20260703-121-v2/`。
- 122 最新正式 `security_policy` credential deep hook 评估证据目录：`results/security_policy/credential-deep-hooks-20260703-122-v2/`。
- audit 模式证据：目标文件和 demo 执行脚本保持可访问，`security_policy_events.audit.jsonl` 包含 `operation=hit`、`result=observed`、`enforce=0`，并覆盖 `event_hook=lsm_file_open/lsm_bprm_check_security/sys_enter_execve/sys_enter_openat/sys_enter_connect/sys_enter_ptrace`；异常证据见 `security_policy_events.anomaly-execve.jsonl`，包含 `operation=anomaly`、`rule_id=burst_execve`、`event_hook=sys_enter_execve`、`threshold/window_ms/hit_count`；服务联动 anomaly 证据见 `security_policy_events.anomaly-rules.jsonl`，包含 `rule_id=burst_connect/burst_openat_sensitive/capability_abuse` 及 `event_hook=sys_enter_connect/lsm_socket_connect/sys_enter_openat/lsm_file_open/lsm_capable`；credential anomaly 证据见 `security_policy_events.credential-anomaly.jsonl` 与 `security_policy_events.credential-hits.jsonl`，包含 `rule_id=credential_churn`、`credential_stage=setuid`、`uid=65534` 和 `cred_gfp` hit 细节。
