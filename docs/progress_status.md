...
阶段 D：Resource Control，状态：`CPU + Memory + IO 自动闭环、target_ref/runtime target 闭环、CPU quota 效果证据和 Redis quota Benchmark 121/122 均已完成`
...
| D. Resource Control | CPU + Memory + IO 自动闭环、target_ref/runtime target 闭环、CPU quota 效果证据和 Redis quota Benchmark 121/122 均已完成 | 正式 `resource_control` 已读取 YAML v2 `controllers + targets + profiles`；cgroup v2 后端已支持 `cpu.weight/cpu.max/cpuset`、`memory.high/memory.low/memory.max` 与 `io.weight/io.max`；写入流程包含旧值读取、值校验、写入、复读验证、`AuditBus` 事件、`ActionJournal` 记录和 Agent stop rollback；121/122 集成测试已验证 background pressure `cpu.max=10000 100000`、`memory.high=1048576`、`io.max wbps=1048576`、`io.weight=default 50`、`memory.events high` 增长、`io.stat wbytes` 增长、写入吞吐下降和旧值恢复；121/122 target 测试已验证显式 cgroup、`container_id`、runtime container name 和 `k8s_pod` 名称解析均只作用于目标 cgroup，非目标 cgroup 不被误改；CPU quota 测试已验证限额后 `usage_rate_ratio=0.1007` 且 `nr_throttled/throttled_usec` 明显增加；Redis quota Benchmark 已验证后台 cgroup CPU 使用率降至默认 noisy 的约 2.4%，并记录 Redis GET/SET RPS 边界 | `docs/resource_control_skill.md`、`tests/integration/test_resource_control.sh`、`tests/integration/test_resource_control_io.sh`、`tests/integration/test_resource_control_target.sh`、`tests/integration/test_resource_control_runtime_target.sh`、`tests/integration/test_resource_control_cpu_quota.sh`、`tests/benchmark/test_resource_control_redis_quota.sh`、`results/resource_control/integration-20260624-160317`、`results/resource_control/integration-20260624-160349`、`results/resource_control/io-20260624-160008`、`results/resource_control/io-20260624-160208`、`results/resource_control/target-20260624-172139`、`results/resource_control/target-20260624-172916`、`results/resource_control/runtime-target-20260624-212403`、`results/resource_control/runtime-target-20260624-212529`、`results/resource_control/cpu-quota-20260625-095030`、`results/resource_control/cpu-quota-20260625-095114`、`results/resource_control/redis-quota-20260625-101307`、`results/resource_control/redis-quota-20260625-101401` |
...
- 121 最新完整质量门禁已通过：`reports/final_quality_gate_20260624_resource_runtime_target.log`。
...
- 121 最新完整质量门禁已通过：`reports/final_quality_gate_20260624_resource_runtime_target.log`，20/20 P0、100 轮 Agent smoke 和 5 轮 doctor 均通过。
...
| D8 | Redis + background CPU quota Benchmark | 121/122 已完成 | `tests/benchmark/test_resource_control_redis_quota.sh` 已在 121/122 记录 Redis GET/SET RPS 与 background cgroup `cpu.stat` 对照；结果显示后台 CPU 使用率降至默认 noisy 的约 2.4%，同时 Redis RPS 未形成提升结论，后续需要继续调参 |
