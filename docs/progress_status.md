...
- SP4 平台完整复核已推进到 sched_ext 自编译内核：123 已启动 `6.6.0-159.4.3.154.oe2403sp4.x86_64-eulerpilot-scx`，`CONFIG_SCHED_CLASS_EXT=y`、`/sys/kernel/sched_ext` 可用；`scripts/check_sp4_env.sh`、`make agent`、`--validate-config`、`--doctor-skills`、`scripts/final_quality_gate.sh` 均通过，新内核下质量门禁为 21/21 P0、100 轮 smoke、5 轮 doctor 通过。
...
- SP4 初始验证已完成：123 机器 `/root/EulerPilot` 在 `d5c3fb3` 上通过 `scripts/check_sp4_env.sh`、`make agent`、Web Console test/lint/build/audit 和 `scripts/final_quality_gate.sh`；当前发行内核未启用 `CONFIG_SCHED_CLASS_EXT`，下一步转入 SP4 内核重编译验证 scx/sched_ext。
