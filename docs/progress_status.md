...
- v3.2 剩余争奖增强重点转为：答辩材料冻结、现场演示压测和 SP4 平台完整复核；SP4 初始验证已通过，下一步进入启用 `CONFIG_SCHED_CLASS_EXT` 的内核重编译。
...
| E. SP4/sched_ext 复核 | SP4 初始验证通过，sched_ext 内核重编译进行中 | 123 已接入 openEuler 24.03 LTS SP4；已启用 cgroup v2/PSI 启动参数并通过最终质量门禁；当前发行内核 `CONFIG_SCHED_CLASS_EXT is not set`，下一步编译启用 sched_ext 的 SP4 内核并复核 scx 路径 | `docs/sp4_validation_plan.md`、`reports/sp4/sp4_initial_validation_20260705-160156.md`、`reports/sp4/final_quality_gate_20260705-160156.log` |
