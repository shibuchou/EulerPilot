...
- SP4 最终质量门禁已刷新：`scripts/final_quality_gate.sh` 通过 22/22 P0、100 轮 Agent smoke 和 5 轮 doctor，日志为 `reports/final_quality_gate_20260720-stage3-performance.log`。
- SP4 frozen-code 争奖性能证据已补强：Redis/Nginx RUNS=10、static-vs-Agent RUNS=10、throughput-first RUNS=10、mixed-adaptive RUNS=10、Agent overhead RUNS=10 与 pressure gradient RUNS=3 均已纳入 `results/final/*20260724-tested-2541464*`。
...
- 121 SP3 强制兼容质量门禁：`reports/final_quality_gate_20260706-quality-121.log`，22/22 P0、100 轮 Agent smoke、5 轮 doctor 均通过；新增 C++ unit tests。当前最终入口以 SP4 `reports/final_quality_gate_20260720-stage3-performance.log` 为准。
- 最终答辩证据压缩入口已完成：`configs/final_evidence_manifest.json`、`scripts/collect_final_evidence.py`、`reports/final_evidence_compact.md`、`reports/final_evidence_compact.json`。当前 `python3 scripts/collect_final_evidence.py --strict` 通过，覆盖 40 个核心证据条目，必需证据缺失为 0、清单警告为 0。
...
- SP4 平台完整复核已完成：123 已启动基于 SP4 官方源码自编译启用 sched_ext 的 `6.6.0-159.4.3.154.oe2403sp4.x86_64-eulerpilot-scx`，`CONFIG_SCHED_CLASS_EXT=y`、`/sys/kernel/sched_ext` 可用；`scripts/check_sp4_env.sh`、`make agent`、`--validate-config`、`--doctor-skills`、`scripts/final_quality_gate.sh`、Redis/Nginx `RUNS=10` frozen-code 对照均通过。
...
- SP4 Redis sched_ext RUNS=10 frozen-code 多轮对照已通过，结果目录：`results/final/redis-scx-compare-20260724-tested-2541464-runs10`。
- SP4 Nginx sched_ext RUNS=10 frozen-code 多轮对照已通过，结果目录：`results/final/nginx-scx-compare-20260724-tested-2541464-runs10`。
- SP4 Redis 压力递增梯度已通过，结果目录：`results/final/redis-pressure-gradient-20260724-tested-2541464-runs3`，覆盖 `STRESS_WORKERS=0/1/2/4/8`。
- SP4 Redis 人工静态 vs Agent 动态对比已用修复脚本重跑：旧目录 `redis-static-vs-agent-20260708-162543` 因只移动 stress-ng 父 PID 已撤下；新目录 `results/final/redis-static-vs-agent-20260724-tested-2541464-runs10` 在目标 cgroup 内启动 stress-ng 及其 worker，manual_static 与 agent_dynamic 均记录 controlled PIDs 与 background cgroup procs。
- SP4 throughput-first 批处理证据已通过，结果目录：`results/final/throughput-first-20260724-tested-2541464-runs10`，`cgroup_throughput_first` 记录 throughput profile hits，`scx_throughput_first` 记录 `class_hits_batch/enqueue_batch/running_batch` 证据；该路径通过显式环境变量启用，默认 Agent 行为不变。
- SP4 mixed-adaptive 完整闭环证据已通过，结果目录：`results/final/mixed-adaptive-20260724-tested-2541464-runs10-lite`，RUNS=10 下 pressure 阶段均触发 ACTIVE，recovery 阶段不再误报 ACTIVE，报告明确 PSI probe 只用于触发证据，不作为净性能收益结论。
- SP4 Agent 控制面开销证据已通过，结果目录：`results/final/agent-overhead-20260724-tested-2541464-runs10`，覆盖 observe-only cgroup、active cgroup 和 active sched_ext 三组 CPU/RSS 采样。
- SP4 最终质量门禁已在 observer 指标、warmup dry-run、sched_ext 默认路径和工程路径收口后再次通过，日志：`reports/final_quality_gate_20260720-stage3-performance.log`，22/22 P0、100 轮 Agent smoke、5 轮 doctor 均通过。
- Web Console v1 已在 SP4 自编译 sched_ext 内核下恢复运行：`/api/health` 正常，`/api/system` 显示 `sched_ext=true`，`/api/evidence/summary` 显示 41 条核心证据、必需缺失 0、警告 0。
...
| G. Benchmark 与冻结材料 | 已完成 | Redis/Nginx 主候选结果、Network QoS 速率误差、Resource Control quota sweep、Policy Engine 演示压测、SP4 frozen-code RUNS=10 复核、Redis pressure gradient、static-vs-agent、throughput-first、mixed-adaptive、Agent overhead、SP4/K8s 旁路验证和 41 条 final evidence compact 已冻结；后续只追加现场彩排截图/录屏，不覆盖主结论 | `docs/stage_g_benchmark_freeze.md`、`configs/final_evidence_manifest.json`、`reports/final_evidence_compact.md`、`results/final/redis-scx-compare-20260724-tested-2541464-runs10`、`results/final/nginx-scx-compare-20260724-tested-2541464-runs10`、`results/final/redis-pressure-gradient-20260724-tested-2541464-runs3`、`results/final/redis-static-vs-agent-20260724-tested-2541464-runs10`、`results/final/throughput-first-20260724-tested-2541464-runs10`、`results/final/mixed-adaptive-20260724-tested-2541464-runs10-lite`、`results/final/agent-overhead-20260724-tested-2541464-runs10`、`results/k8s/sp4-validation-20260708-023552` |
