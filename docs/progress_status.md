...
- v3.2 第一阶段已把真实 container runtime target 从 blocked 推进到双机 pass：121/122 均安装 `docker-engine` 与 `podman`，Docker 18.09 daemon 在当前 cgroup v2 环境下因 `Devices cgroup isn't mounted` 无法作为主验证 runtime，Podman 4.9.4 可用并作为本阶段真实 runtime 验证入口。
- 为避免依赖 Docker Hub，121/122 均使用 openEuler `busybox` 包和 glibc 依赖构造本地 `localhost/eulerpilot-busybox:latest` 镜像，真实 runtime 测试不依赖外网镜像拉取。
- readiness 刷新后，121/122 均为 `container_runtime_ready=1`、`kubernetes_ready=0`，结果目录分别为 `results/resource_control/runtime-readiness-20260630-podman-121` 和 `results/resource_control/runtime-readiness-20260630-podman-122`。
- 真实 Podman container target 已在 121/122 通过：121 `results/resource_control/real-runtime-target-20260630-podman-121-final2`，122 `results/resource_control/real-runtime-target-20260630-podman-122-final2`；两端均验证 `target_ref=real_container`、真实容器 cgroup、`cpu.max=10000 100000`、`memory.high=1048576`、applied/restored 审计和 rollback。
- `TargetResolver` 已修正 Podman/systemd cgroup v2 场景：`type: container + container_name + runtime` 会优先通过 runtime PID 读取 `/proc/<pid>/cgroup`，定位到真实进程所在的 `.../libpod-*.scope/container` 子 cgroup；纯 fake `container_id` 测试仍走 cgroup 扫描，避免误探测 Docker daemon。
- fake runtime target 回归已在 121/122 再次通过：121 `results/resource_control/runtime-target-20260630-113310`，122 `results/resource_control/runtime-target-20260630-113354`。
- 121 最新质量门禁通过：`reports/final_quality_gate_20260630-v32-podman-121.log`，21/21 P0、100 轮 Agent smoke、5 轮 doctor 均通过。
- Kubernetes/真实 Pod target 与 Pod host veth 仍后置：当前缺 `kubectl`、`eulerpilot-lab` namespace 和 demo Pod，不从最终争奖路线删除，继续作为 v3.2 下一优先级。
...
阶段 D：Resource Control，状态：`CPU + Memory + IO 自动闭环、target_ref/runtime target 闭环、Redis/Nginx 单项与混合 quota sweep、多资源组合 profile 证据已完成；真实 Podman container target 已在 121/122 转 pass，真实 Pod/K8s 仍等待 kubectl/lab 环境`

阶段 F：Kubernetes 与跨 Agent 联动，状态：`v3.1 第二条跨 Skill 联动已完成；Kubernetes/真实 runtime 后置 v3.2 第一优先级`
...
| D9 | 真实 runtime readiness 诊断 | 121/122 已完成，Podman ready / K8s lab missing | `tests/integration/test_resource_control_runtime_readiness.sh` 已在 121/122 刷新诊断；两端 `podman ps` 可用、`container_runtime_ready=1`，Docker daemon 因 cgroup v2 devices controller 问题不可作为主验证 runtime，Kubernetes lab 仍缺 `kubectl` 与 demo Pod |
...
| D14 | 真实 runtime / Pod target 演示入口 | 真实 Podman container target 双机 pass；真实 Pod blocked | `tests/integration/test_resource_control_real_runtime_target.sh` 已在 121/122 使用本地 Podman 镜像通过，验证 `type: container` 的 `target_ref`、真实容器 cgroup、`cpu.max/memory.high` 写入、审计和 rollback；`tests/integration/test_resource_control_real_pod_target.sh` 仍等待 `kubectl + eulerpilot-lab` demo Pod |
