...
## SP4/Kubernetes 最新验证批次（2026-07-08）
...
- SP4 最终质量门禁已刷新：`scripts/final_quality_gate.sh` 通过 22/22 P0、100 轮 Agent smoke 和 5 轮 doctor，日志为 `reports/final_quality_gate_20260720-path-closeout.log`。
...
- 121 历史质量门禁：`reports/final_quality_gate_20260706-quality-121.log`，22/22 P0、100 轮 Agent smoke、5 轮 doctor 均通过；新增 C++ unit tests。当前最终入口以 SP4 `reports/final_quality_gate_20260720-path-closeout.log` 为准。
...
- SP4 Redis 人工静态 vs Agent 动态对比已用修复脚本重跑：旧目录 `redis-static-vs-agent-20260708-162543` 因只移动 stress-ng 父 PID 已撤下；新目录 `results/final/redis-static-vs-agent-20260720-114909` 在目标 cgroup 内启动 stress-ng 及其 worker，manual_static 与 agent_dynamic 均记录 controlled PIDs 与 background cgroup procs。
- SP4 最终质量门禁已在 observer 指标、warmup dry-run、sched_ext 默认路径和工程路径收口后再次通过，日志：`reports/final_quality_gate_20260720-path-closeout.log`，22/22 P0、100 轮 Agent smoke、5 轮 doctor 均通过。
- Web Console v1 已在 SP4 自编译 sched_ext 内核下恢复运行：`/api/health` 正常，`/api/system` 显示 `sched_ext=true`，`/api/evidence/summary` 显示 37 条核心证据、必需缺失 0、警告 0。
...
| E. SP4/sched_ext 复核 | SP4 sched_ext 自编译内核与 RUNS=5 workload 对照已通过 | 123 已接入 openEuler 24.03 LTS SP4；发行内核默认未启用 `CONFIG_SCHED_CLASS_EXT`，当前已自编译并启动 `6.6.0-159.4.3.154.oe2403sp4.x86_64-eulerpilot-scx`，`CONFIG_SCHED_CLASS_EXT=y`、`/sys/kernel/sched_ext` 可用；新内核下 `check_sp4_env`、质量门禁、Web Console Evidence、v3.1 `security_policy -> policy_engine -> resource_control + network_qos -> rollback` repeat 10、Redis smoke、Redis PSI ACTIVE probe、Redis RUNS=5 compare、Nginx RUNS=5 compare、Redis pressure gradient 均通过；Redis static-vs-agent 已用修复脚本 RUNS=5 重跑 | `docs/sp4_validation_plan.md`、`reports/sp4/sp4_sched_ext_validation_20260705-211407.md`、`reports/sp4/final_quality_gate_scx_workload_20260706-1214.log`、`results/policy_engine/security-network-resource-20260705-211407`、`results/reports/redis-scx-smoke-20260706-092724`、`results/final/redis-scx-psi-probe-20260706-100857`、`results/final/redis-scx-compare-20260708-150702`、`results/final/nginx-scx-compare-20260708-152602`、`results/final/redis-pressure-gradient-20260708-153811`、`results/final/redis-static-vs-agent-20260720-114909` |
...
| G. Benchmark 与冻结材料 | 已完成 | Redis/Nginx 主候选结果、Network QoS 速率误差、Resource Control quota sweep、Policy Engine 演示压测、SP4 RUNS=5 复核、Redis pressure gradient、SP4/K8s 旁路验证和 37 条 final evidence compact 已冻结；后续只追加现场彩排截图/录屏，不覆盖主结论 | `docs/stage_g_benchmark_freeze.md`、`configs/final_evidence_manifest.json`、`reports/final_evidence_compact.md`、`results/final/redis-scx-compare-20260708-150702`、`results/final/nginx-scx-compare-20260708-152602`、`results/final/redis-pressure-gradient-20260708-153811`、`results/final/redis-static-vs-agent-20260720-114909`、`results/k8s/sp4-validation-20260708-023552` |
...
- 历史质量门禁 `reports/final_quality_gate_20260629_policy_engine.log` 曾通过 21/21 P0；当前最终质量门禁已升级为 SP4 `reports/final_quality_gate_20260720-path-closeout.log`，22/22 P0、100 轮 Agent smoke 和 5 轮 doctor 均通过。
