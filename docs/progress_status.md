...
- Web Console v1 已在 SP4 sched_ext 内核下恢复运行：`/api/health` 正常，`/api/system` 显示 `sched_ext=true`，`/api/evidence/summary` 显示 28 条核心证据、必需缺失 0、警告 0。
...
- v3.2 剩余争奖增强重点转为：答辩材料冻结、现场演示压测、SP4 sched_ext 多轮重复实验和最终多端一致性收口。
...
| E. SP4/sched_ext 复核 | SP4 sched_ext 自编译内核与 workload 对照已通过第一轮 | 123 已接入 openEuler 24.03 LTS SP4；发行内核默认未启用 `CONFIG_SCHED_CLASS_EXT`，当前已自编译并启动 `6.6.0-159.4.3.154.oe2403sp4.x86_64-eulerpilot-scx`，`CONFIG_SCHED_CLASS_EXT=y`、`/sys/kernel/sched_ext` 可用；新内核下 `check_sp4_env`、质量门禁、Web Console Evidence、v3.1 `security_policy -> policy_engine -> resource_control + network_qos -> rollback` repeat 10、Redis smoke、Redis PSI ACTIVE probe、Redis compare 和 Nginx compare 均通过 | `docs/sp4_validation_plan.md`、`reports/sp4/sp4_sched_ext_validation_20260705-211407.md`、`reports/sp4/final_quality_gate_scx_20260705-205406.log`、`results/policy_engine/security-network-resource-20260705-211407`、`results/reports/redis-scx-smoke-20260706-092724`、`results/final/redis-scx-psi-probe-20260706-100857`、`results/final/redis-scx-compare-20260706-101505`、`results/final/nginx-scx-compare-20260706-101928` |
