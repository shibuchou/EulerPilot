...
阶段 C：Security Agent 正式化，状态：`BPF LSM + syscall tracing + 多目标 target_map 最小闭环已完成`
...
| C. Security Agent | 双 LSM + 四类 syscall tracing + 多目标 target_map 最小闭环已完成 | 正式 `security_policy` 注册名已落地；YAML v2 path/exec_path target、用户态下发最多 8 项 BPF `target_map`、audit BPF attach 不阻断 + `lsm_file_open/lsm_bprm_check_security/sys_enter_execve/sys_enter_openat/sys_enter_connect/sys_enter_ptrace` ringbuf observed hit、enforce BPF LSM blocked hit、双动态 `/tmp` 目标验证、rollback 无残留已在 121/122 通过；`security_policy_demo` 保留兼容；容器 target 绑定和规则级事件标识尚未完成 | `docs/security_policy_skill.md`、`agent/skills/security_policy/README.md`、`tests/integration/test_security_policy.sh`、`demo/security_policy_demo/README.md` |
...
- 121 完整质量门禁已通过：`reports/final_quality_gate_20260621_security_multitarget.log`。
...
- 121 最新完整质量门禁已通过：`reports/final_quality_gate_20260621_security_multitarget.log`，17/17 P0、100 轮 Agent smoke 和 5 轮 doctor 均通过。
