...
- SP4 最终质量门禁已在 sched_ext workload 收口后再次通过，日志：`reports/sp4/final_quality_gate_scx_workload_20260706-1214.log`，21/21 P0、100 轮 Agent smoke、5 轮 doctor 均通过。
- Web Console v1 已在 SP4 sched_ext 内核下恢复运行：`/api/health` 正常，`/api/system` 显示 `sched_ext=true`，`/api/evidence/summary` 显示 32 条核心证据、必需缺失 0、警告 0。
...
| G. Benchmark 与冻结材料 | 已完成 | Redis/Nginx 主候选结果、Network QoS 速率误差、Resource Control quota sweep、Policy Engine 演示压测、SP4 RUNS=3 复核和 32 条 final evidence compact 已冻结；后续只追加现场彩排截图/录屏，不覆盖主结论 | `docs/stage_g_benchmark_freeze.md`、`configs/final_evidence_manifest.json`、`reports/final_evidence_compact.md`、`results/final/redis-scx-compare-20260706-115029`、`results/final/nginx-scx-compare-20260706-120547` |
