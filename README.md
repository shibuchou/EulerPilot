...
- Security Policy 阶段 C 最小闭环：
  ...
  - BPF LSM：`file_open`、`bprm_check_security`、`socket_connect`、`ptrace_traceme`、`capable`、`task_fix_setuid`、`task_fix_setgid` 与 `task_fix_setgroups` enforce
  ...
  - credential policy：`lsm/task_fix_setuid`、`lsm/task_fix_setgid` 与 `lsm/task_fix_setgroups`，已验证仅目标 cgroup 内 setuid/setgid/setgroups credential 转换被拒绝，并输出 `uid/euid/suid/setuid_flags`、`gid/egid/sgid/setgid_flags`、`group_count/old_group_count`
  ...
  - target scope：path、path_prefix、file_access、exec_path、exec_prefix、socket endpoint、ptrace/capability/setuid/setgid/setgroups cgroup scope、显式 cgroup、PID 自动解析、container_id cgroup tree 解析、container runtime name 解析、Kubernetes Pod 名称解析
...
- Security scoped credential 121：`/root/EulerPilot/results/security_policy/integration-20260623-214457`
- Security scoped credential 122：`/root/EulerPilot/results/security_policy/integration-20260623-214850`
